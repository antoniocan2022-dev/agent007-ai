...
import { approveRevenueExecution, getRevenueExecutionQueue, prepareRevenueExecution, REVENUE_EXECUTION_ACTIONS, type RevenueExecutionAction } from '@/lib/revenue-execution'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (operation === 'prepare') {
      ...
      if (!REVENUE_EXECUTION_ACTIONS.includes(action)) {
        return NextResponse.json({ ok: false, error: 'Unsupported revenue execution action.', allowedActions: REVENUE_EXECUTION_ACTIONS }, { status: 400 })
      }

      const idempotencyKey = String(body?.idempotencyKey ?? '').trim()
      if (!idempotencyKey) return NextResponse.json({ ok: false, error: 'idempotencyKey is required.' }, { status: 400 })

      const prepared = await prepareRevenueExecution(userId, {
        action,
        idempotencyKey,
    ...
    return NextResponse.json({ ok: false, error: 'Unknown operation. Use prepare or approve.' }, { status: 400 })
  } catch (error: any) {
    return NextResponse.json({ ok: false, error: error?.message ?? 'Revenue execution action failed.' }, { status: 500 })
  }
}
