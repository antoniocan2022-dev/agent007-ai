...
function isConversationalHistoryRow(row: PersistedConversationRow): boolean {
  if (row.role === 'user') return true
  if (row.role !== 'assistant') return false
  return Boolean(row.content?.trim()) && !containsInternalArtifactToken(row.content)
}
...
function buildConversationModule(input: { currentUserMessage: string; persistedMessages: readonly PersistedConversationRow[]; recentMessageLimit: number; relevantOlderLimit: number }): { messages: Array<{ role: CeoContextRole; content: string }>; recent: PersistedConversationRow[]; relevantOlder: PersistedConversationRow[]; summarizedCount: number; excludedContaminatedAssistantMessages: number } {
  ...
  const excludedContaminatedAssistantMessages = sourceRows.filter((row) => row.role === 'assistant' && !isConversationalHistoryRow(row)).length
  const rows = uniqueRows(sourceRows.filter(isConversationalHistoryRow).sort((a, b) => asTimestamp(a.createdAt) - asTimestamp(b.createdAt)))
