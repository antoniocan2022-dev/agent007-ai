...
async function streamDownloadToFile(key: string, destPath: string, expectedSize: number, checksum?: { algorithm: string; value: string }): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  try {
    while (true) {
      ...
      if (bytesWritten > MAX_SYNCHRONOUS_INGEST_BYTES) {
        throw new Error(`Object exceeds the ${Math.round(MAX_SYNCHRONOUS_INGEST_BYTES / 1024 / 1024)}MB synchronous ingestion limit (discovered mid-download; declared size did not match).`)
      }
      hash.update(value)
...
export async function ingestOciDocument(input: RemoteIngestInput): Promise<RemoteIngestResult> {
  ...
  try {
    const downloaded = await streamDownloadToFile(key, tmpPath, input.size, input.checksum)
