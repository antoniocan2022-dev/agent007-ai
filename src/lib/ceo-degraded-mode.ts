...
export async function buildCeoDegradedResponse(input: { objective: string; intent: CeoIntent; responseAction?: ResponseAction; selfReflectionKind?: SelfReflectionKind; reason: string; failureReason?: CeoFailureReason; missionId?: string; contextualEvidence?: string; priorConversation?: readonly PersistedConversationRow[]; recall?: MemoryRecall; domain?: string; operation?: string; conversationState?: CeoConversationState; resolvedReferences?: readonly ConversationReference[]; recoveredExternalEvidence?: boolean; partnerIntelligence?: PartnerIntelligenceSummary; executiveState?: ExecutiveBusinessState; leadershipLedger?: readonly LeaderPerformanceRecord[]; strategicHorizon?: StrategicHorizonView; documentComprehensionSynthesis?: string; documentComprehensionCoverage?: string }): Promise<DegradedResponse> {
  ...
  const natural = buildNaturalRecoveryResponse({ objective: input.objective, action: input.responseAction, priorConversation: safeConversationRows(input.priorConversation ?? []), recoveredContext, isSuppliedByCaller: Boolean(suppliedContext), intent: input.intent, conversationState, resolvedReferences: input.resolvedReferences, documentComprehensionSynthesis: input.documentComprehensionSynthesis, documentComprehensionCoverage: input.documentComprehensionCoverage })
