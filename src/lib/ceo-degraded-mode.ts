...
function inferFailureReason(message: string): CeoFailureReason {
  if (timeoutOrDeadline.test(message)) return 'execution_timeout'
...
const timeoutOrDeadline = /timeout|timed out|deadline/i
...
function requiresDecisionGradeAbstention(input: { objective: string; failureReason: CeoFailureReason }): boolean {
  ...
  return riskClassForDomain('public_equity', input.objective) === 'HIGH' && HIGH_RISK_OBJECTIVE_RE.test(input.objective)
}

function buildRiskAbstention(objective: string, reason: string): DegradedResponse {
  return {
