...
function inferFailureReason(message: string): CeoFailureReason {
  if (/timeout|timed out|deadline/i.test(message)) return 'execution_timeout'
  if (/provider|model|llm/i.test(message)) return /unavailable|no provider/i.test(message) ? 'provider_unavailable' : 'provider_error'
  if (/evidence|source|research/i.test(message)) return /insufficient/i.test(message) ? 'evidence_insufficient' : 'evidence_unavailable'
  if (/claim.{0,40}consisten|contradiction/i.test(message)) return 'claim_consistency_failure'
  if (/quality|objective coverage/i.test(message)) return 'quality_failure'
  if (/tool/i.test(message)) return /unavailable|missing/i.test(message) ? 'tool_unavailable' : 'tool_error'
  ...
  if (/production|release|traffic|deployment/i.test(message)) return 'production_verification_failure'
  return 'unknown'
}

const HIGH_RISK_OBJECTIVE_RE = /\b(?:stock(?:s)?|share(?:s)?|equity|ticker|invest(?:ing|ment)?|buy|sell|hold|portfolio|security|cybersecurity|legal|law|regulat(?:ion|ory)|compliance|financial\s+(?:decision|advice|recommendation)|production\s+(?:change|deploy|deployment)|deploy)\b/i
...
export function buildRiskAbstention(objective: string, reason: string, failureReason: CeoFailureReason = 'evidence_insufficient'): DegradedResponse {
  return {
    ...
    sourceKeys: [],
    failureReason,
    recoveredCapability: 'evidence',
    content: `ABSTAINED_REQUIRED_EVIDENCE\n\nI did not complete the evidence required for this high-risk request, so I am not going to give you a decision-grade recommendation from memory or an incomplete execution path.\n\nRequest: ${objective.slice(0, 800)}\nReason: ${reason.slice(0, 500)}\n\nThe safe next step is to complete the required live evidence acquisition and verification before making the decision.`,
  }
}

function requiresDecisionGradeAbstention(input: { objective: string; failureReason: CeoFailureReason; domain?: string }): boolean {
  if (input.failureReason === 'evidence_insufficient' || input.failureReason === 'evidence_unavailable' || input.failureReason === 'production_verification_failure') return true
  const domain = input.domain ?? (/\b(?:stock(?:s)?|equity|ticker|invest(?:ing|ment)?|buy|sell|hold)\b/i.test(input.objective) ? 'public_equity' : 'general_web')
  return riskClassForDomain(domain, input.objective) === 'HIGH' && HIGH_RISK_OBJECTIVE_RE.test(input.objective)
}
...
export async function buildCeoDegradedResponse(input: {
  ...
  if (requiresDecisionGradeAbstention({ objective: input.objective, failureReason, domain: input.domain })) {
    return buildRiskAbstention(input.objective, input.reason, failureReason)
  }
  ...
  if (input.intent !== 'self_assessment') {
    const natural = buildNaturalRecoveryResponse({
      objective: input.objective,
      action: input.responseAction,
      priorConversation: input.priorConversation,
      recoveredContext,
      isSuppliedByCaller: Boolean(suppliedContext),
    })
    if (natural) {
      return {
