...
Each stage: Leader runs -> Super Agent verifies (0-100 score) -> if score < 85,
retry with corrections (max 3 rounds) -> next stage -> CEO presents final report.
...
// Phase 2 of the CEO Conversation Kernel migration (external audit, 2026-09-19), issue 9 -- recorded
// here, not fixed, per the audit's own framing ("architectural, not immediate"):
//
// runOrchestrator still owns two genuinely different responsibilities in one function: dispatching real
// tool calls against live external systems (the ACT stage), and, for messages the classifiers above
// route to 'direct', generating the conversational answer itself via its own LLM call inside the tool
// loop -- a second, independent answer-generation path alongside ceo-cognitive-lifecycle.ts's, not a
// delegation to it. The full canonical Option 3 architecture (Understand -> Remember -> Decide ->
// Reason/Research -> Act -> Verify -> Respond) implies the orchestrator should eventually be
// execution-only -- dispatch tools, report real outcomes, and nothing else -- with every conversational
// answer generated by exactly one place.
//
// This is a real architectural gap, not a documentation-only one, and it is deliberately NOT attempted
// in this same Phase 2 pass: runOrchestrator is the single highest-blast-radius file in the CEO pipeline
// (every operational_orchestrator-owned request, plus every tool the CEO can take a real-world action
// through, flows through it), and separating its dual role touches how 'direct' vs 'dispatch'
// classification, streaming, recovery budgets, and tool-loop state interact -- exactly the kind of
// same-day, high-risk, low-test-coverage change Stage 1a's investigation already flagged and declined to
// bundle with a same-day migration step (see ceo-operational-direct-response.ts's own header comment).
// The other 8 issues in this pass are bounded, independently testable, and each only narrows an existing
// gap without changing what owns the request; this one would change ownership itself. Treat this comment
// as the standing TODO for that decomposition, to be scoped and executed as its own reviewed change --
// not silently dropped, and not rushed into the same pass as everything else here.
