...
export type EvidenceRequirement = 'none' | 'internal_state' | 'memory' | 'live_system' | 'database' | 'external_web' | 'multi_source'
...
export interface CeoResponseDecisionEnvelope { candidate: CeoResponseCandidate; quality: CeoQualityDecision }
export interface FinalResponseProvenance { finalizationId: string; finalResponseHash: string; finalContentLength: number; candidateId?: string; candidateHash?: string; qualityDecisionId?: string; sanitized: boolean; rejected: boolean }
export interface QualityResult { decision: QualityDecision; evidenceState: EvidenceState; verificationStatus: VerificationStatus; checks: { nonEmpty: boolean; contractValid: boolean; objectiveCoverage: boolean; internalConsistency: boolean; evidenceDiscipline: boolean; actionableStructure: boolean }; evidenceScope?: EvidenceScope; evidenceFreshness?: EvidenceFreshness; claimScopes?: EvidenceScope[]; contextContinuity?: ContextContinuitySummary; conversationQuality?: ConversationQualitySummary; finalResponseProvenance?: FinalResponseProvenance; failureReason?: CeoFailureReason; failure?: CeoFailure; reasons: string[] }
