...
function isMeaningfulPrediction(value: string | null | undefined): value is string { if (!value) return false; const text = value.replace(/\s+/g, ' ').trim(); if (text.length < 24 || text.length > 2000) return false; return !/^(?:it depends|unclear|unknown|not sure|cannot predict|can\s*not predict|there is not enough information|insufficient information|n\/a)\.?$/i.test(text) }
async function capturePredictedOutcome(input: { objective: string; recommendedAction: string; responseAction: string }): Promise<string | null> {
  try {
    ...
    const result = await runCanonicalLlm({ messages: [
      { role: 'system', content: 'Produce exactly one meaningful, observable predicted outcome for this recommendation. It must describe a measurable or externally observable result and a reasonable horizon when the objective provides one. Do not invent facts, guarantees, fabricated numbers, or certainty. If no defensible outcome can be predicted from the supplied information, return exactly NOT_PREDICTABLE.' },
      { role: 'user', content: `Objective: ${input.objective.slice(0, 3500)}\nAction class: ${input.responseAction}\nRecommended action: ${input.recommendedAction.slice(0, 7000)}` },
    ], taskType: 'reasoning', executionClass: 'fast', temperature: 0, maxTokens: 120, timeoutMs: 5000, maxProviderAttempts: 1 })
    const prediction = result.content.trim().replace(/^[-*]\s+/, '').replace(/^Prediction:\s*/i, '').trim()
    return /^NOT_PREDICTABLE\.?$/i.test(prediction) || !isMeaningfulPrediction(prediction) ? null : prediction.slice(0, 2000)
  } catch { return null }
}

export function buildRecommendationRecord(input: { correlationId: string; objective: string; responseAction: string; decisionRationale?: string; predictedOutcome?: string | null; predictionHorizon?: string | null; predictionEligibility?: PredictionEligibility; recommendedAction?: string; recordedAt?: number }): CeoRecommendation {
...
export async function recordCeoRecommendation(input: Parameters<typeof buildRecommendationRecord>[0]): Promise<CeoRecommendation> {
  ...
  const predictedOutcome = initial.predictionEligibility === 'ELIGIBLE' ? (initial.predictedOutcome ?? await capturePredictedOutcome({ objective: initial.objective, recommendedAction: initial.recommendedAction, responseAction: initial.responseAction })) : null
  const record = predictedOutcome && isMeaningfulPrediction(predictedOutcome) ? { ...initial, predictedOutcome, predictionStatus: 'PREDICTED' as const } : initial
  ...
  if (existing) {
    ...
    const priorEligibility = prior.predictionEligibility ?? predictionEligible(prior.responseAction)
    const immutableMismatch = prior.correlationId !== record.correlationId || prior.objective !== record.objective || prior.responseAction !== record.responseAction || prior.recommendedAction !== record.recommendedAction || priorEligibility !== record.predictionEligibility
    if (immutableMismatch) throw new Error(`Conflicting CEO recommendation already exists for ${record.correlationId}.`)
    if (!prior.predictedOutcome && record.predictedOutcome) { await db.memory.update({ where: { key }, data: { value: JSON.stringify(record), category: 'ceo_recommendation' } }); return record }
    return { ...prior, predictionEligibility: priorEligibility, predictionStatus: prior.predictionStatus ?? (priorEligibility === 'NOT_APPLICABLE' ? 'NOT_APPLICABLE' : prior.predictedOutcome ? 'PREDICTED' : 'NOT_CAPTURED') }
  }
  ...
  try { const { startContinuousLoop } = await import('./ceo-continuous-loop'); await startContinuousLoop({ recommendationId: record.recommendationId, evidence: [`recommendation:${record.recommendationId}`] }) } catch (error) { console.warn('[ceo-recommendation] continuous-loop initialization failed:', error instanceof Error ? error.message.slice(0, 180) : String(error)) }
  return record
}
...
export async function recordRecommendationAction(input: { recommendationId: string; description: string; status?: CeoRecommendationAction['status']; observedAt?: number | null }): Promise<CeoRecommendationAction> {
  ...
  const { db } = await import('./db'); const key = `ceo_recommendation_action:${action.actionId}`; const existing = await db.memory.findUnique({ where: { key } })
  if (existing) { const prior = JSON.parse(existing.value) as CeoRecommendationAction; if (prior.recommendationId !== action.recommendationId || prior.description !== action.description) throw new Error(`Conflicting CEO recommendation action already exists for ${action.actionId}.`); return prior }
  await db.memory.create({ data: { key, value: JSON.stringify(action), category: 'ceo_recommendation_action' } }); return action
}
...
export function buildObservedRecommendationOutcome(input: { recommendationId: string; observedOutcome: string; actualResult: string; observedAt?: number; source: string; metadata?: Record<string, unknown> }): ObservedRecommendationOutcome {
  ...
  const observedAt = input.observedAt ?? Date.now(); if (!Number.isFinite(observedAt)) throw new Error('Observed recommendation outcome observedAt must be a finite timestamp.')
...
export async function recordObservedRecommendationOutcome(input: Parameters<typeof buildObservedRecommendationOutcome>[0]): Promise<ObservedRecommendationOutcome> {
  const outcome = buildObservedRecommendationOutcome(input); const { db } = await import('./db'); const key = `ceo_observed_outcome:${outcome.outcomeId}`; const existing = await db.memory.findUnique({ where: { key } })
  if (existing) { const prior = JSON.parse(existing.value) as ObservedRecommendationOutcome; if (prior.recommendationId !== outcome.recommendationId || prior.actualResult !== outcome.actualResult || prior.source !== outcome.source) throw new Error(`Conflicting observed recommendation outcome already exists for ${outcome.outcomeId}.`); return prior }
  await db.memory.create({ data: { key, value: JSON.stringify(outcome), category: 'ceo_observed_outcome' } }); return outcome
}
...
export function calculateRecommendationPredictionError(recommendation: CeoRecommendation | null, outcome: ObservedRecommendationOutcome | null): RecommendationPredictionError | null {
  ...
  const status = recommendation.predictionStatus ?? (recommendation.predictedOutcome ? 'PREDICTED' : 'NOT_CAPTURED')
  if (status !== 'PREDICTED' || !recommendation.predictedOutcome) return { recommendationId: recommendation.recommendationId, predictionStatus: status, errorMagnitude: null, direction: 'unknown', explanation: status === 'NOT_APPLICABLE' ? 'This recommendation was explicitly marked not applicable for prediction.' : 'No meaningful prediction was captured, so prediction error cannot be measured.' }
  const predicted = recommendation.predictedOutcome.toLowerCase().trim(); const actual = outcome.actualResult.toLowerCase().trim()
  if (predicted === actual) return { recommendationId: recommendation.recommendationId, predictionStatus: status, errorMagnitude: 0, direction: 'matched', explanation: 'Observed actual result matches the captured prediction text.' }
  const predictedNumbers = [...predicted.matchAll(/-?\d+(?:\.\d+)?/g)].map((match) => Number(match[0])).filter(Number.isFinite); const actualNumbers = [...actual.matchAll(/-?\d+(?:\.\d+)?/g)].map((match) => Number(match[0])).filter(Number.isFinite)
  if (predictedNumbers.length && actualNumbers.length) { const errorMagnitude = Math.abs(actualNumbers[0] - predictedNumbers[0]); return { recommendationId: recommendation.recommendationId, predictionStatus: status, errorMagnitude, direction: actualNumbers[0] === predictedNumbers[0] ? 'matched' : actualNumbers[0] > predictedNumbers[0] ? 'better_than_predicted' : 'worse_than_predicted', explanation: `Numeric prediction error is ${errorMagnitude}.` } }
  return { recommendationId: recommendation.recommendationId, predictionStatus: status, errorMagnitude: null, direction: 'unknown', explanation: 'Prediction and actual result differ, but no reliable numeric error can be calculated.' }
}
