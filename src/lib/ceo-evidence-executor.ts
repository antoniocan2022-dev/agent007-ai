...
export interface ExternalEvidenceExecution {
...
function toolContext(): ToolContext {
  return { attachments: [], language: 'en' }
}
...
async function fetchJson<T>(url: string): Promise<T> {
  const response = await fetch(url, {
    headers: {
      ...
      'User-Agent': process.env.SEC_USER_AGENT?.trim() || DEFAULT_SEC_UA,
    },
    redirect: 'follow',
...
async function getSecTickerMap(): Promise<SecTickerMap> {
  if (cachedSecTickers && Date.now() - cachedSecTickers.loadedAt < SEC_TICKER_TTL_MS) return cachedSecTickers.value
  const data = await fetchJson<SecTickerMap>('https://www.sec.gov/files/company_tickers.json')
  const normalized: SecTickerMap = {}
  for (const item of Object.values(data)) {
    const ticker = String(item.ticker ?? '').trim().toUpperCase()
    if (ticker) normalized[ticker] = item
  }
...
const FACT_CANDIDATES: Array<{ key: string; label: string }> = [
  ...
  { key: 'SalesRevenueNet', label: 'Sales' },
  { key: 'CashAndCashEquivalentsAtCarryingValue', label: 'Cash' },
  { key: 'Assets', label: 'Assets' },
  { key: 'Liabilities', label: 'Liabilities' },
  { key: 'LongTermDebtCurrent', label: 'Current debt' },
  { key: 'LongTermDebtNoncurrent', label: 'Long-term debt' },
  { key: 'NetIncomeLoss', label: 'Net income' },
]

function latestUnit(units?: Record<string, SecFactUnit[]>): SecFactUnit | null {
  const candidates = Object.values(units ?? {}).flat().filter((item) => typeof item.val === 'number' && item.filed)
...
async function fetchSecSource(ticker: string): Promise<EvidenceSource | null> {
  ...
  if (!item?.cik_str) return null
  const cik = String(item.cik_str).padStart(10, '0')
  const url = `https://data.sec.gov/api/xbrl/companyfacts/CIK${cik}.json`
  const payload = await fetchJson<SecFacts>(url)
  const usGaap = payload.facts?.['us-gaap'] ?? {}
  const lines: string[] = [`SEC Company Facts for ${ticker.toUpperCase()} — ${payload.entityName ?? item.title}`]
  let latestFiled: string | undefined
  for (const candidate of FACT_CANDIDATES) {
    const fact = latestUnit(usGaap[candidate.key]?.units)
    if (!fact) continue
    if (fact.filed && (!latestFiled || fact.filed > latestFiled)) latestFiled = fact.filed
    lines.push(`${candidate.label}: ${fact.val} (${fact.form ?? 'filing'}, filed ${fact.filed}${fact.fp ? `, ${fact.fp}` : ''})`)
  }
  if (lines.length === 1) return null
  const publishedAt = latestFiled ? Date.parse(`${latestFiled}T00:00:00Z`) : undefined
  return createEvidenceSource({
    url,
    title: `${ticker.toUpperCase()} SEC Company Facts`,
...
function titleFromSearchResult(result: ToolResult, url: string): string {
  const line = result.result.split('\n').find((candidate) => candidate.includes(url))
  if (!line) return url
  return line.replace(/^[0-9]+\.\s*/, '').split('\n')[0].replace(/\*\*/g, '').trim() || url
}
...
async function executeSearch(query: EvidenceQuery): Promise<{ result: ToolResult; sources: EvidenceSource[] }> {
  ...
  const sources = urls.map((url, index) => createEvidenceSource({
    url,
    title: titleFromSearchResult(result, url),
    sourceType: query.sourcePreference === 'market' ? 'market_data' : query.sourcePreference === 'company' ? 'company_ir' : 'web',
    sourceTier: sourceTierForUrl(url),
    retrievedAt,
    text: result.result.slice(0, 6000),
    id: `${query.id}-${index + 1}`,
  }))
...
async function readPages(urls: string[]): Promise<EvidenceSource[]> {
  const outputs = await Promise.all(urls.map(async (url, index) => {
    ...
    return createEvidenceSource({
      url,
      title: result.preview.replace(/^Read page(?: \(via fallback\))?:\s*/i, '').slice(0, 240) || url,
      sourceType: 'page',
      sourceTier: sourceTierForUrl(url),
      retrievedAt: Date.now(),
      text: result.result,
...
export async function executeExternalEvidencePlan(plan: ExternalEvidencePlan): Promise<ExternalEvidenceExecution> {
  const failures: string[] = []
  const queries = plan.queries.slice(0, plan.maxSearchQueries)
  ...
  if (pagesToRead.length) {
    try { pageSources = await readPages(pagesToRead) }
    catch (error) { failures.push(`page_reader: ${error instanceof Error ? error.message : String(error)}`) }
  }
  ...
  if (plan.profile === 'public_equity') {
    ...
    const secResults = await Promise.all(tickers.map(async (ticker) => {
      try { return await fetchSecSource(ticker) }
      catch (error) {
        failures.push(`SEC ${ticker}: ${error instanceof Error ? error.message : String(error)}`)
        return null
      }
    }))
  ...
  return {
    ...
    successfulQueries: searchResults.filter((entry) => entry.sources.length > 0).length,
    pageReads: pageSources.length,
    secSources: secSources.length,
    failures,
  }
}