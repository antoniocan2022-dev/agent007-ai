...
async function getSecTickerMap(signal?: AbortSignal): Promise<SecTickerMap> { throwIfCeoRequestAborted(signal); if (cachedSecTickers && Date.now() - cachedSecTickers.loadedAt < SEC_TICKER_TTL_MS) return cachedSecTickers.value; const data = await fetchJson<SecTickerMap>('https://www.sec.gov/files/company_tickers.json', signal); const normalized: SecTickerMap = {}; for (const item of Object.values(data)) { const ticker = String(item.ticker ?? '').trim().toUpperCase(); if (ticker) normalized[ticker] = item } cachedSecTickers = { loadedAt: Date.now(), value: normalized }; return normalized }
...
async function fetchSecSource(ticker: string, signal?: AbortSignal): Promise<EvidenceSource | null> {
  const map = await getSecTickerMap(signal), item = map[ticker.toUpperCase()]; if (!item?.cik_str) return null
  const cik = String(item.cik_str).padStart(10, '0'), url = `https://data.sec.gov/api/xbrl/companyfacts/CIK${cik}.json`, payload = await fetchJson<SecFacts>(url, signal), usGaap = payload.facts?.['us-gaap'] ?? {}
...
async function executeSearch(query: EvidenceQuery, signal?: AbortSignal): Promise<{ result: ToolResult; sources: EvidenceSource[] }> {
  throwIfCeoRequestAborted(signal)
  const result = await dispatch('web_search', cacheBypassArgs({ query: query.query, num: 6, recency_days: query.recencyDays }), signal); if (!result.ok) return { result, sources: [] }
...
async function readPages(urls: string[], signal?: AbortSignal): Promise<EvidenceSource[]> {
  const outputs = await Promise.all(urls.map(async (url, index) => { throwIfCeoRequestAborted(signal); const result = await dispatch('page_reader', cacheBypassArgs({ url }), signal); if (!result.ok) return null; return createEvidenceSource({ url, title: result.preview.replace(/^Read page(?: \(via fallback\))?:\s*/i, '').slice(0, 240) || url, sourceType: 'page', sourceTier: sourceTierForUrl(url), retrievedAt: Date.now(), text: result.result, id: `PAGE-${index + 1}` }) }))
