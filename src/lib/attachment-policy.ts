...
export const ATTACHMENT_STATUSES = [
  'INITIATED', 'UPLOADING', 'UPLOADED', 'QUARANTINED', 'PROCESSING', 'READY', 'FAILED', 'ABORTED',
] as const
...
const CODE_EXTENSIONS = new Set([
  'js','jsx','ts','tsx','py','go','rs','java','c','h','cpp','hpp','cc','cs','php','rb','swift','kt','kts',
  'sql','sh','bash','zsh','ps1','html','css','scss','sass','xml','yaml','yml','json','toml','ini','env','md','mdx','txt','log',
])
...
export function normalizeAttachmentName(name: string): string {
  ...
  const basename = normalized.replace(/[\\/]/g, '_').replace(/^\.+/, '') || 'attachment'
  return basename.slice(0, MAX_FILENAME_CHARS)
}
...
export function validateAttachmentRequest(input: { filename: string; mimeType?: string | null; size: number }) {
  ...
  if (input.size > MAX_ATTACHMENT_BYTES) throw new Error('Attachment exceeds the 10 GiB per-file limit.')
...
export function partSizeForSize(size: number): number {
  return Math.min(MULTIPART_PART_BYTES, Math.ceil(size / MAX_MULTIPART_PARTS))
}
