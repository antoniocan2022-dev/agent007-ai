...
export async function createInvoice(input:InvoiceInput):Promise<string>{await assertVentureForAttach(input.ventureId);const customer=await db.$queryRaw<Array<{id:string;ventureId:string|null}>>`SELECT "id","ventureId" FROM "Customer" WHERE "id"=${input.customerId} LIMIT 1`;if(!customer[0]||customer[0].ventureId!==input.ventureId)throw new Error('Invoice customer must belong to the same venture.');if(input.subscriptionId){const subscription=await db.$queryRaw<Array<{id:string;ventureId:string}>>`SELECT "id","ventureId" FROM "Subscription" WHERE "id"=${input.subscriptionId} LIMIT 1`;if(!subscription[0]||subscription[0].ventureId!==input.ventureId)throw new Error('Invoice subscription must belong to the same venture')}if(input.transactionId){const transaction=await db.$queryRaw<Array<{id:string;ventureId:string|null}>>`SELECT "id","ventureId" FROM "Transaction" WHERE "id"=${input.transactionId} LIMIT 1`;if(!transaction[0]||transaction[0].ventureId!==input.ventureId)throw new Error('Invoice transaction must belong to the same venture.')}const amount=positiveAmount(input.amount,'Invoice amount');const currency=clean(input.currency??'USD').toUpperCase();if(!/^[A-Z]{3}$/.test(currency))throw new Error('Invoice currency must be ISO-4217 alpha-3.');const id=`inv_${input.provider}_${input.providerInvoiceId??`${input.ventureId}_${input.customerId}_${amount}_${Date.now()}`}`;await db.$executeRaw`INSERT INTO "Invoice" ("id","ventureId","customerId","subscriptionId","transactionId","provider","providerInvoiceId","status","amount","currency","dueAt","paidAt","rawPayload") VALUES (${id},${input.ventureId},${input.customerId},${input.subscriptionId??null},${input.transactionId??null},${clean(input.provider)},${input.providerInvoiceId??null},${input.status??'open'},${amount},${currency},${isoOrNull(input.dueAt,'dueAt')},${isoOrNull(input.paidAt,'paidAt')},${input.rawPayload??null}) ON CONFLICT ("id") DO UPDATE SET "status"=EXCLUDED."status","amount"=EXCLUDED."amount","currency"=EXCLUDED."currency","dueAt"=EXCLUDED."dueAt","paidAt"=EXCLUDED."paidAt","updatedAt"=CURRENT_TIMESTAMP`;return id}
