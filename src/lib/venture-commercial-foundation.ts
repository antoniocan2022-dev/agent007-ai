...
export async function createOrGetVenture(input: Omit<VentureRecord,'id'|'createdAt'|'updatedAt'>): Promise<VentureRecord> {
  ...
  await db.$executeRaw`INSERT INTO "Venture" ("id","ventureKey","businessUnitId","ownerUserId","name","type","description","targetMarket","pricingModel","status","productionState") VALUES (${ventureKey},${ventureKey},${input.businessUnitId},${ownerUserId},${name},${clean(input.type)},${clean(input.description)},${clean(input.targetMarket)},${clean(input.pricingModel)},${input.status},${input.productionState}) ON CONFLICT ("ventureKey") DO UPDATE SET "businessUnitId"=COALESCE(EXCLUDED."businessUnitId","Venture"."businessUnitId"),"ownerUserId"=EXCLUDED."ownerUserId","name"=EXCLUDED."name","type"=EXCLUDED."type","description"=EXCLUDED."description","targetMarket"=EXCLUDED."targetMarket","pricingModel"=EXCLUDED."pricingModel","status"=EXCLUDED."status","productionState"=EXCLUDED."productionState","updatedAt"=CURRENT_TIMESTAMP`
