...
import { isRetrospectiveConversationRequest, isObjectiveContinuationSignal, isObjectiveConfirmationSignal, CONTEXTUAL_REFERENCE_RE } from './ceo-conversational-signals'
...
function latestContinuableObjective(context?: CanonicalConversationContext): string | undefined {
  if (!context || !isObjectiveContinuationSignal(context.currentMessage)) return undefined
  ...
  return resolution.resolvedObjective?.trim() || resolution.resolvedText?.trim() || undefined
}
