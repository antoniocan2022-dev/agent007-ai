...
import { PortfolioExperimentStatus } from './portfolio-intelligence-types'
...
export async function recordExperimentPaymentAttribution(input: {
  experimentId: string
  business: PortfolioBusiness
  variant: string
  ventureId: string
  transactionId: string
  evidenceId: string
}): Promise<ExperimentPaymentAttribution> {
  ...
  if (![PortfolioExperimentStatus.APPROVED, PortfolioExperimentStatus.RUNNING, PortfolioExperimentStatus.COMPLETED].includes(experiment.status)) {
    throw new Error(`Experiment ${input.experimentId} is not in an attributable state.`)
  }
  ...
  const record: ExperimentPaymentAttribution = {
    attributionId,
  ...
  await db.memory.upsert({
    where: { key: attributionId },
    update: { value: JSON.stringify(record), category: 'portfolio_experiment_attribution' },
    create: { key: attributionId, category: 'portfolio_experiment_attribution', value: JSON.stringify(record) },
  })
  return record
}
