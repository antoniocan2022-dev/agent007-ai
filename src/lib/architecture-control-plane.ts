...
/** Canonical enterprise authority levels. */
...
const LEADERS = new Set([
  'scout', 'aurora', 'vertex', 'quantum', 'echo', 'forge', 'pulse', 'quill', 'prism',
  'legal', 'banker', 'hunt', 'developer', 'cybersecurity_a', 'cybersecurity_r',
  'trader', 'revenue', 'external_uptime_monitor', 'qa_monitor',
])
...
/**
 * Universal hierarchy invariant:
 * CEO → VID → LEADER → SPECIALIST/TOOL.
 * The caller may never forge a different authority level than the canonical identity resolves to.
 */
export function assertDelegationAllowed(request: DelegationRequest): void {
  ...
  if (actualActorLevel !== request.actorLevel) {
    throw new Error(`Hierarchy violation: actor identity ${actor} resolves to ${actualActorLevel}, not ${request.actorLevel}.`)
  }
  if (actualTargetLevel !== request.targetLevel) {
    throw new Error(`Hierarchy violation: target identity ${target} resolves to ${actualTargetLevel}, not ${request.targetLevel}.`)
  }

  if (request.actorLevel === 'CEO') {
    if (request.targetLevel !== 'VID' || !VID_IDS.has(target)) {
      throw new Error(`Hierarchy violation: CEO may delegate only to VID; attempted target=${target}.`)
    }
    return
  }
  if (request.actorLevel === 'VID') {
    if (request.targetLevel !== 'LEADER' || !LEADERS.has(target)) {
  ...
  if (request.actorLevel === 'LEADER') {
    if (request.targetLevel !== 'SPECIALIST' && request.targetLevel !== 'TOOL') {
...
function artifactEventKey(artifactId: string, type: ArtifactStatus, actor: string, metadata = ''): string {
  return `architecture:artifact:event:${stableId('event', artifactId, type, actor, metadata)}`
}
...
function validateArtifactInput(input: Omit<ArtifactRecord, 'artifactId' | 'createdAt' | 'status' | 'verifiedAt' | 'verificationSource'>): void {
  ...
  if (input.supersedes && input.supersedes === input.artifactId) throw new Error('Artifact cannot supersede itself.')
}

async function writeArtifactEvent(event: ArtifactLedgerEvent): Promise<void> {
  await db.memory.upsert({
    where: { key: event.eventId },
    update: { value: JSON.stringify(event), category: 'architecture_artifact_event' },
    create: { key: event.eventId, value: JSON.stringify(event), category: 'architecture_artifact_event' },
  })
}

export async function registerArtifact(
  input: Omit<ArtifactRecord, 'artifactId' | 'createdAt' | 'status' | 'verifiedAt' | 'verificationSource'> & Partial<Pick<ArtifactRecord, 'artifactId' | 'createdAt'>>,
  ...
  await writeArtifactEvent({
    eventId: artifactEventKey(artifactId, 'PRODUCED', record.producer),
    artifactId,
    type: 'PRODUCED',
  ...
  if (record.supersedes) {
    await supersedeArtifact(record.supersedes, record.producer, artifactId)
  }
  return record
}
...
export async function verifyArtifact(artifactId: string, actor: string, source: string): Promise<ArtifactRecord> {
  ...
  if (record.status === 'REJECTED' || record.status === 'SUPERSEDED') {
    throw new Error(`Artifact cannot be verified from terminal status ${record.status}.`)
  }
  ...
  await writeArtifactEvent({
    eventId: artifactEventKey(artifactId, 'VERIFIED', actor, source),
    artifactId,
    type: 'VERIFIED',
    actor: actor.trim(),
...
export async function listArtifacts(input?: { ventureId?: string; missionId?: string; status?: ArtifactStatus; limit?: number }): Promise<ArtifactRecord[]> {
  ...
  return rows.map((row) => {
    try { return JSON.parse(row.value) as ArtifactRecord } catch { return null }
  }).filter((artifact): artifact is ArtifactRecord => Boolean(artifact))
    .filter((artifact) => !input?.ventureId || artifact.ventureId === input.ventureId)
    .filter((artifact) => !input?.missionId || artifact.missionId === input.missionId)
...
export function buildOutcomeId(input: Pick<BusinessOutcomeRecord, 'ventureId' | 'missionId' | 'type' | 'transactionId' | 'customerId' | 'amount' | 'occurredAt'>): string {
  const businessKey = input.type === 'KPI_SNAPSHOT' ? input.occurredAt : input.transactionId ?? input.customerId ?? `${input.missionId ?? ''}:${input.amount ?? ''}`
...
export function validateBusinessOutcome(input: Omit<BusinessOutcomeRecord, 'outcomeId'>): string[] {
  ...
  if (!Number.isFinite(Date.parse(input.occurredAt))) errors.push('occurredAt must be a valid timestamp.')
  ...
  if ((input.type === 'TRANSACTION' || input.type === 'REVENUE_RECOGNIZED' || input.type === 'REFUND') && !input.transactionId?.trim()) {
    errors.push(`${input.type} requires transactionId evidence.`)
  }
...
export async function recordBusinessOutcome(input: Omit<BusinessOutcomeRecord, 'outcomeId'> & Partial<Pick<BusinessOutcomeRecord, 'outcomeId'>>): Promise<BusinessOutcomeRecord> {
  const normalized: Omit<BusinessOutcomeRecord, 'outcomeId'> = {
    ...input,
  ...
  const record = { ...normalized, outcomeId }
...
export async function listBusinessOutcomes(input?: { ventureId?: string; type?: BusinessOutcomeType; limit?: number }): Promise<BusinessOutcomeRecord[]> {
  const rows = await db.memory.findMany({ where: { category: 'architecture_business_outcome' }, orderBy: { createdAt: 'desc' }, take: Math.min(Math.max(input?.limit ?? 100, 1), 5000) })
  return rows.map((row) => {
    try { return JSON.parse(row.value) as BusinessOutcomeRecord } catch { return null }
  }).filter((item): item is BusinessOutcomeRecord => Boolean(item))
    .filter((item) => !input?.ventureId || item.ventureId === input.ventureId)
...
export const VENTURE_001_CONTRACT: Omit<VentureControlContract, 'createdAt' | 'updatedAt'> = {
  ventureId: 'venture_001', version: 1, status: 'ACTIVE',
...
const GENERIC_VENTURE_CONTRACT: Omit<VentureControlContract, 'ventureId' | 'createdAt' | 'updatedAt'> = {
  version: 1, status: 'DRAFT', allowedActions: ['research', 'draft_product', 'create_artifact', 'verify_artifact', 'analyze_kpi'],
  forbiddenActions: ['transfer_funds', 'sign_legal_contract', 'change_business_entity', 'delete_canonical_record'],
  approvalRequiredAbove: { spend: 0 }, requiredEvidence: [], ownerApprovalRequiredFor: ['public_launch', 'production_deploy', 'fund_transfer', 'legal_commitment', 'retire_venture'],
}

function validateContract(contract: VentureControlContract): void {
  const overlap = new Set(contract.allowedActions.map(normalizeId)).intersection?.(new Set(contract.forbiddenActions.map(normalizeId)))
  if (overlap && overlap.size > 0) throw new Error(`Venture contract has overlapping allowed/forbidden actions: ${[...overlap].join(', ')}`)
...
export interface VentureControlContractTemplateInput {
  allowedActions?: string[]
  forbiddenActions?: string[]
  approvalRequiredAbove?: Record<string, number>
  requiredEvidence?: string[]
  ownerApprovalRequiredFor?: string[]
  status?: VentureControlContract['status']
}
...
export async function createVentureControlContract(ventureId: string, input?: VentureControlContractTemplateInput): Promise<VentureControlContract> {
  ...
  const now = new Date().toISOString()
  const contract: VentureControlContract = {
    ventureId: normalized,
    version: 1,
    status: input?.status ?? base.status,
    allowedActions: [...(base.allowedActions ?? [])],
    forbiddenActions: [...(base.forbiddenActions ?? [])],
    approvalRequiredAbove: { ...(base.approvalRequiredAbove ?? {}) },
    requiredEvidence: [...(base.requiredEvidence ?? [])],
    ownerApprovalRequiredFor: [...(base.ownerApprovalRequiredFor ?? [])],
    createdAt: now,
    updatedAt: now,
  }
  validateContract(contract)
