 ...
 * lifecycle decision and applies only eligible lifecycle mutations.
 */
...
export interface VentureDecisionResult {
  ...
  score: number | null
  reasons: string[]
  scorecard: { opportunity?: ReturnType<typeof calculateOpportunityScore>; health?: ReturnType<typeof calculateVentureHealth> }
}
...
function spendAllowed(requestedSpend = 0, monthlyCommittedSpend = 0): boolean {
  return requestedSpend >= 0 && requestedSpend <= CEO_VENTURE_MANDATE.maximumSingleSpendWithoutApproval && monthlyCommittedSpend + requestedSpend <= CEO_VENTURE_MANDATE.maximumMonthlyBurnWithoutApproval
}
...
function deriveLifecycleDecision(business: Business, health: ReturnType<typeof calculateVentureHealth>): VentureDecision {
  ...
  if (health.decision === 'scale') return 'scale'
  if (health.decision === 'optimize') return 'optimize'
  if (health.decision === 'experiment') return 'experiment'
...
export async function recordLaunchVerification(businessId: string, verification: LaunchVerification): Promise<VentureEvidenceRecord> {
  return recordVentureEvidence({ businessId, kind: 'launch', source: verification.source, statement: verification.statement, confidence: verification.confidence, verified: true, createdAt: verification.verifiedAt })
}
...
export async function evaluateVentureDecision(input: VentureDecisionInput): Promise<VentureDecisionResult> {
  ...
  if (input.opportunity) {
    ...
    if (business.lifecycle === 'proposed') {
      ...
      return { engineVersion: VENTURE_DECISION_ENGINE_VERSION, businessId: business.businessId, lifecycle: business.lifecycle, decision, confidence: opportunity.confidence, autonomousEligible: decision === 'build' && reasons.length === mandateErrors.length && spendAllowed(input.requestedSpend, input.monthlyCommittedSpend), irreversibleActionBlocked: true, score: opportunity.score, reasons: [...reasons, ...opportunity.blockingReasons], scorecard: { opportunity } }
    }
  }
