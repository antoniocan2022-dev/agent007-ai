...
export interface EvidenceEntityCoverage { entity: string; sourceIds: string[]; sourceCount: number; sufficient: boolean }
export interface EvidenceBundle { scope: EvidenceScope; profile: EvidenceProfile; operation?: EvidenceOperation; createdAt: number; sources: EvidenceSource[]; claims: EvidenceClaimCandidate[]; freshness: EvidenceFreshness; contextText: string; sufficient: boolean; contradictions: readonly ContradictionRecord[]; requiredEntities?: string[]; entityCoverage?: EvidenceEntityCoverage[] }
...
export function buildEvidenceBundle(input: { profile: EvidenceProfile; operation?: EvidenceOperation; sources: EvidenceSource[]; scope?: EvidenceScope; minimumSources?: number; minimumTierOneSources?: number; requiredEntities?: string[] }): EvidenceBundle {
  ...
  const requiredEntities = [...new Set((input.requiredEntities ?? []).map((entity) => entity.trim().toUpperCase()).filter(Boolean))]
  const entityCoverage: EvidenceEntityCoverage[] = requiredEntities.map((entity) => {
    const matching = sources.filter((source) => (source.relatedEntities ?? []).some((related) => related.trim().toUpperCase() === entity))
    const fresh = matching.some((source) => { const age = createdAt - source.retrievedAt; return age >= 0 && age <= profileMaxAge })
    return { entity, sourceIds: matching.map((source) => source.id), sourceCount: matching.length, sufficient: matching.length > 0 && fresh }
  })
  const entityCoverageSatisfied = !requiredEntities.length || entityCoverage.every((coverage) => coverage.sufficient)
  const sufficient = sources.length >= minimumSources && sources.filter((source) => source.sourceTier <= 1).length >= minimumTierOneSources && entityCoverageSatisfied
  ...
  return { scope: input.scope ?? 'external_web', profile: input.profile, operation: input.operation, createdAt, sources, claims, freshness: { observedAt, maxAgeMs: profileMaxAge }, contextText, sufficient, contradictions, ...(requiredEntities.length ? { requiredEntities, entityCoverage } : {}) }
}
...
export function renderEvidenceBundleForPrompt(bundle: EvidenceBundle): string {
  ...
  const entityLine = bundle.requiredEntities?.length ? `Entity coverage: ${bundle.entityCoverage?.map((coverage) => `${coverage.entity}=${coverage.sufficient ? 'covered' : 'missing'}(${coverage.sourceCount} source${coverage.sourceCount === 1 ? '' : 's'})`).join('; ')}` : ''
  return [`EVIDENCE BUNDLE: ${bundle.profile}`, `Observed: ${new Date(bundle.freshness.observedAt).toISOString()}`, `Sufficient: ${bundle.sufficient ? 'yes' : 'no'}`, ...(entityLine ? [entityLine] : []), freshnessLine, ...(contradictionsBlock ? [contradictionsBlock] : []), 'Source markers are authoritative. Do not create citations that are not present in this bundle.', 'Claims are unverified until the governed verification step upgrades them.', bundle.contextText].join('\n\n')
}