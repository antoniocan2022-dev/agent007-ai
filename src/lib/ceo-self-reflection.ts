...
const CASUAL_CHECKIN_RE = /^(?:how(?:'s|\s+is)\s+(?:it|everything|things?|everything\s+going)|how\s+are\s+(?:you|things?)(?:\s+doing)?|how\s+is\s+(?:agent007|the\s+(?:system|ceo|agent))\s+doing|you\s+(?:good|okay|alright)|what(?:'s|\s+is)\s+new(?:\s+with\s+you)?)[!.?\s]*$/i
const PERFORMANCE_RE = /\b(?:improving|getting\s+better|performance|performing|progress|progressing|better|worse|declining|evolving|evolution|learning|developing|growth|how\s+have\s+you\s+been)\b/i
...
export function classifyCeoSelfReflection(text: string): SelfReflectionClassification {
  ...
  if (EXPLICIT_OPERATION_RE.test(normalized) || RESEARCH_RE.test(normalized) || MISSION_RE.test(normalized)) {
  ...
  // A short self-referential question with no operational target is still much
  // more likely to be a reflective check-in than deep external work.
  if (normalized.length <= 220 && /\?\s*$/.test(normalized)) {
    return { kind: 'casual_checkin', isSelfReflective: true, reason: 'Short self-referential non-operational question.' }
  }
