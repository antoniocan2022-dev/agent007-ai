...
function ledgerContentHash(input:{missionId:string;version:number;title:string;status:string;previousHash:string|null;sources:StoredSource[];claims:HashClaim[]}):string {
  ...
  return sha256({missionId:input.missionId,version,inputTitle:input.title,status,previousHash:input.previousHash,sources,claims})
}
...
function inputIdentity(input:EvidenceLedgerInput,status:string,previousHash:string|null,prepared:StoredSource[]):string{return sha256({missionId:input.missionId,title:input.title,status,previousHash,sources:prepared.map((s)=>({key:sourceKey(s),provider:s.provider,sourceUrl:s.sourceUrl,rawEvidenceRef:s.rawEvidenceRef,rawEvidenceHash:s.rawEvidenceHash,requestHash:s.requestHash})).sort((a,b)=>a.key.localeCompare(b.key)),claims:hashClaimsFromInput(input,prepared)})}
function storedIdentity(existing:{missionId:string;title:string;status:string;previousHash:string|null;Source:StoredSource[];Claim:Array<{claimKey:string;claimText:string;classification:string;confidence:number;verificationStatus:string;notes:string|null;sourceId:string|null}>}):string {const sourceById=new Map(existing.Source.map((s)=>[s.id,s]));const claims=existing.Claim.map((c)=>({claimKey:c.claimKey,claimText:c.claimText,classification:c.classification,confidence:c.confidence,verificationStatus:c.verificationStatus,sourceKey:c.sourceId?sourceKey(sourceById.get(c.sourceId)!):null,notes:c.notes??null})).sort((a,b)=>a.claimKey.localeCompare(b.claimKey));return sha256({missionId:existing.missionId,title:existing.title,status:existing.status,previousHash:existing.previousHash,sources:existing.Source.map((s)=>({key:sourceKey(s),provider:s.provider,sourceUrl:s.sourceUrl,rawEvidenceRef:s.rawEvidenceRef,rawEvidenceHash:s.rawEvidenceHash,requestHash:s.requestHash})).sort((a,b)=>a.key.localeCompare(b.key)),claims})}
