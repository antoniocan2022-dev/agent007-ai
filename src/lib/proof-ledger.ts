...
function claimHash(c:EvidenceClaimInput,prepared:StoredSource[]):HashClaim {
...
function validateLedgerInput(input:EvidenceLedgerInput):void {
  ...
  input.claims.forEach((c)=>{assertNonEmpty(`claim[${c.claimKey||'unnamed'}].claimKey`,c.claimKey);assertNonEmpty(`claim[${c.claimKey||'unnamed'}].claimText`,c.claimText);assertConfidence(c.confidence);if(c.sourceIndex!==undefined&&(!Number.isInteger(c.sourceIndex)||c.sourceIndex<0||c.sourceIndex>=input.sources.length))throw new Error(`Claim ${c.claimKey} references invalid source index ${c.sourceIndex}.`);if(!claimKeys.add(c.claimKey))throw new Error(`Duplicate evidence claim key: ${c.claimKey}.`)})
}
function inputLogicalIdentity(input:EvidenceLedgerInput,status:string,previousHash:string|null,prepared:StoredSource[]):string{return logicalLedgerHash(input.missionId,input.title,status,previousHash,normalizedSourceHash(prepared),hashClaimsFromInput(input,prepared))}
function storedLogicalIdentity(existing:{missionId:string;title:string;status:string;previousHash:string|null;Source:StoredSource[];Claim:Array<{claimKey:string;claimText:string;classification:string;confidence:number;verificationStatus:string;notes:string|null;sourceId:string|null}>}):string {const sourceById=new Map(existing.Source.map((s)=>[s.id,s]));const claims=existing.Claim.map((c)=>({claimKey:c.claimKey,claimText:c.claimText,classification:c.classification,confidence:c.confidence,verificationStatus:c.verificationStatus,sourceKey:c.sourceId?sourceKey(sourceById.get(c.sourceId)!):null,notes:c.notes??null}));return logicalLedgerHash(existing.missionId,existing.title,existing.status,existing.previousHash,normalizedSourceHash(existing.Source),claims)}
...
export async function persistEvidenceLedger(input:EvidenceLedgerInput){
  ...
  const contentHash=ledgerContentHash({missionId:input.missionId,version,title:input.title,status,previousHash,sources:prepared,claims:hashClaimsFromInput(input,prepared)})
