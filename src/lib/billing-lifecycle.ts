...
export async function settleInvoiceFromTransaction(invoiceId: string, transactionId: string): Promise<InvoiceSettlementResult> {
  ...
  if (invoice.status === 'paid') {
    ...
    const verified = await assertRealSucceededTransaction({ ventureId: invoice.ventureId, transactionId: requestedTransactionId, customerId: invoice.customerId, amount: Number(invoice.amount), currency: invoice.currency })
  ...
  const tx = await assertRealSucceededTransaction({ ventureId: invoice.ventureId, transactionId: requestedTransactionId, customerId: invoice.customerId, amount: Number(invoice.amount), currency: invoice.currency })
...
export async function cancelSubscription(subscriptionId: string): Promise<void> {
  const rows = await db.$queryRaw<Array<{ id:string; ventureId:string; customerId:string; status:string }>>`SELECT "id","ventureId","customerId","status" FROM "Subscription" WHERE "id"=${subscriptionId} LIMIT 1`
...
export async function activateSubscriptionFromPaidInvoice(subscriptionId: string, nextPeriodStart: string, nextPeriodEnd: string): Promise<void> {
  ...
  const paidInvoice = await db.$queryRaw<Array<{ id:string; ventureId:string; customerId:string; transactionId:string|null }>>`
    SELECT "id","ventureId","customerId","transactionId" FROM "Invoice" WHERE "subscriptionId"=${subscriptionId} AND "status"='paid' ORDER BY "paidAt" DESC NULLS LAST LIMIT 1
  `
  ...
  if (paidInvoice[0].ventureId !== subscription.ventureId) throw new Error(`Subscription ${subscriptionId} has a paid invoice from a different venture.`)
  if (paidInvoice[0].customerId !== subscription.customerId) throw new Error(`Subscription ${subscriptionId} has a paid invoice for a different customer.`)
  await assertRealSucceededTransaction({
    ventureId: paidInvoice[0].ventureId,
