 ...
 */
import { db } from './db'
import { getObservabilityMetrics, type ObservabilityMetrics } from './mission-telemetry'
...
export interface OperationalKpiInput {
  now: Date
  missions: Pick<ObservabilityMetrics, 'totalMissions' | 'missionSuccessRate'>
  businesses: Business[]
  incomeEntries: IncomeFact[]
...
export function isReferenceVenture(business: Business): boolean {
  return normalizedName(business.name) === normalizedName(VENTURE_001_REFERENCE.name)
}
...
export function calculateBusinessHealth(business: Business): number {
  ...
  const lifecycleHealth: Record<BusinessLifecycle, number> = {
    proposed: 30,
    validated: 55,
    launched: 75,
    active: 90,
...
export function calculateOperationalKpis(input: OperationalKpiInput): OperationalKpiSnapshot {
  ...
  const terminalMissions = Math.round(input.missions.totalMissions * ((input.missions.missionSuccessRate ?? 0) / 100))
  const failedMissions = Math.max(0, input.missions.totalMissions - terminalMissions)
  const activeMissions = 0
  ...
  return {
    ...
    missions: {
      ...
      completed: terminalMissions,
      failed: failedMissions,
      total: input.missions.totalMissions,
      successRate: input.missions.missionSuccessRate === null ? null : Number(input.missions.missionSuccessRate.toFixed(1)),
    },
    ventures: {
    ...
    commercial: {
      ...
      transactions30d: verifiedIncome.length,
      customers: Math.max(0, Math.floor(input.customerCount)),
      openOpportunities: Math.max(0, Math.floor(input.opportunityCount)),
    },
    referenceVenture001: {
...
export async function loadOperationalKpis(now = new Date()): Promise<OperationalKpiSnapshot> {
  const [observability, businesses, incomeEntries, customerCount, opportunityCount] = await Promise.all([
    getObservabilityMetrics(),
    getPortfolio().catch(() => []),
    db.incomeEntry.findMany({
      where: { date: { gte: new Date(now.getTime() - KPI_WINDOW_DAYS * 24 * 60 * 60 * 1000) } },
      select: { amount: true, source: true, notes: true, date: true },
    }).catch(() => []),
