/**
 * Venture OS Autonomy Manager — Upgrade 14.
 *
 * This is the canonical orchestration boundary between the 5–13 control plane
 * and long-running operation. It is intentionally side-effect conservative:
 * a heartbeat may inspect, claim, reconcile, and schedule work, but it may not
 * manufacture business outcomes or bypass owner-gated venture actions.
 */
...
async function acquireLease(now: Date, runId: string): Promise<boolean> {
  ...
  try {
    if (existing) {
      await db.memory.update({ where: { key: LEASE_KEY }, data: { value, category: 'venture_autonomy_lease' } })
    } else {
      await db.memory.create({ data: { key: LEASE_KEY, value, category: 'venture_autonomy_lease' } })
    }
...
async function persistRun(run: AutonomyManagerRun): Promise<void> {
  await db.memory.upsert({
    where: { key: `${RUN_CATEGORY}:${run.runId}` },
    update: { value: JSON.stringify(run), category: RUN_CATEGORY },
    create: { key: `${RUN_CATEGORY}:${run.runId}`, value: JSON.stringify(run), category: RUN_CATEGORY },
  })
}
...
async function listDueWork(now: Date, limit: number): Promise<AutonomyWorkItem[]> {
  ...
  return rows
    .map((row) => parseJson<AutonomyWorkItem>(row.value))
    .filter((item): item is AutonomyWorkItem => Boolean(item))
    .filter((item) => (item.status === 'QUEUED' || (item.status === 'CLAIMED' && item.claimedAt && Date.parse(item.claimedAt) + LEASE_TTL_MS <= now.getTime())))
    .filter((item) => Number.isFinite(Date.parse(item.nextRunAt)) && Date.parse(item.nextRunAt) <= now.getTime())
...
async function updateWork(item: AutonomyWorkItem): Promise<void> {
  ...
  await db.memory.upsert({
    where: { key: `${WORK_CATEGORY}:${item.workId}` },
    update: { value: JSON.stringify(item), category: WORK_CATEGORY },
    create: { key: `${WORK_CATEGORY}:${item.workId}`, value: JSON.stringify(item), category: WORK_CATEGORY },
  })
}

/** Enqueue durable autonomous work exactly once by idempotency key. */
export async function enqueueAutonomyWork(input: {
  ventureId: string
  missionId?: string | null
  action: string
  idempotencyKey: string
  ...
  const item: AutonomyWorkItem = {
    ...
    nextRunAt: input.nextRunAt ? new Date(input.nextRunAt).toISOString() : now,
    claimedAt: null,
    lastError: null,
    createdAt: now,
    updatedAt: now,
...
/**
 * Execute one bounded manager heartbeat. Execution itself remains represented
 * by durable work items; this layer only claims work after governance/readiness
 * checks. Integrations may consume CLAIMED items through their own governed
 * executor without weakening this manager's invariants.
 */
export async function runAutonomyManagerTick(options: AutonomyManagerOptions = {}): Promise<AutonomyManagerRun> {
  ...
  const run: AutonomyManagerRun = {
    runId, version: MANAGER_VERSION, startedAt: started.toISOString(), finishedAt: started.toISOString(), status: 'COMPLETED', leaseAcquired: false,
  ...
  try {
    ...
    for (const ventureId of ventureIds) {
      ...
      try {
        ...
        if (readiness.status === 'READY') run.venturesReady++
        else run.venturesBlocked++
        ...
        for (const item of due.filter((candidate) => candidate.ventureId === ventureId)) {
          if (readiness.status !== 'READY') {
            item.status = 'BLOCKED'
            item.lastError = readiness.blockingReasons.join('; ')
            run.workBlocked++
          ...
          item.status = 'CLAIMED'
          item.attempts += 1
          item.claimedAt = started.toISOString()
          item.lastError = null
          await updateWork(item)
          run.workClaimed++
        }
