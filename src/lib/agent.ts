...
import type { AttachmentMeta, ToolResult } from '@/lib/tools'
 ...
 * The retired Z.AI/OpenAI-specific runtime was removed. All model execution
 * now flows through the canonical provider control plane.
 */
...
export async function getSystemPrompt(): Promise<string> {
  ...
  cachedSystemPrompt = SYSTEM_PROMPT.replaceAll('${TOOL_COUNT}', '0')
  try {
...
export interface AgentEventEmit { (event: 'thought' | 'tool_call' | 'tool_result' | 'token' | 'memory_update' | 'error' | 'heartbeat' | 'progress' | 'reasoning', data: any): Promise<void> | void }
...
export interface AgentRunResult { finalAnswer: string; steps: Array<{ id: string; thought?: string; toolName?: string; toolArgs?: any; toolResult?: ToolResult; startedAt: number; finishedAt?: number }>; persistedAssistantMessageId: string }
...
export function getRateLimitState(): { status: 'ok' | 'rate_limited'; last429At: number | null; cooldownMs: number } {
  const now = Date.now(); const cooldownUntil = RATE_LIMIT_INFO.last429At ? RATE_LIMIT_INFO.last429At + RATE_LIMIT_COOLDOWN_MS : 0
...
export function wasTruncatedByLength(completion: any): boolean { return (completion?.choices?.[0]?.finish_reason ?? completion?.choices?.[0]?.message?.finish_reason) === 'length' }
export function validateToolArgs(rawArgsString: string | undefined): { ok: boolean; args: any; error?: string } {
  ...
  try { return { ok: true, args: JSON.parse(rawArgsString) } }
  catch (e: any) { return { ok: false, args: {}, error: `Invalid JSON in tool args: ${e?.message ?? 'parse error'}. Raw: "${rawArgsString.slice(0, 200)}"` } }
}
...
export interface Parsed { thought?: string; tool?: { name: string; args: any }; dispatch?: { agentId: string; task: string }; textAfterTool: string; textBeforeTool: string; raw: string }
export function parseAssistant(content: string): Parsed {
  const thought = content.match(THOUGHT_RE)?.[1]?.trim(); const toolMatch = content.match(TOOL_RE); const dispatchMatch = content.match(DISPATCH_SUBAGENT_RE); let tool: Parsed['tool']; let dispatch: Parsed['dispatch']; let textBeforeTool = content.replace(THOUGHT_RE, '').trim(); let textAfterTool = ''
  if (toolMatch) {
    const name = (toolMatch[1] ?? '').trim(); let args: any = {}; const raw = (toolMatch[2] ?? '').trim()
    if (raw) { try { args = JSON.parse(raw) } catch { const recovered: Record<string, string> = {}; const re = /"([^"]+)"\s*:\s*"([^"]*)"/g; let match: RegExpExecArray | null; while ((match = re.exec(raw))) recovered[match[1]] = match[2]; args = recovered } }
    tool = { name, args }; const idx = content.indexOf(toolMatch[0]); textBeforeTool = content.slice(0, idx).replace(THOUGHT_RE, '').trim(); textAfterTool = content.slice(idx + toolMatch[0].length).trim()
    if (name === 'dispatch_subagent') { const agentId = String(args?.id ?? args?.agentId ?? '').trim(); const task = String(args?.task ?? args?.goal ?? '').trim(); if (agentId) dispatch = { agentId, task } }
  } else if (dispatchMatch) {
    const agentId = (dispatchMatch[1] ?? '').trim(); const task = (dispatchMatch[2] ?? '').trim(); if (agentId) { tool = { name: 'dispatch_subagent', args: { id: agentId, task } }; dispatch = { agentId, task }; const idx = content.indexOf(dispatchMatch[0]); textBeforeTool = content.slice(0, idx).replace(THOUGHT_RE, '').trim(); textAfterTool = content.slice(idx + dispatchMatch[0].length).trim() }
  }
...
export async function buildHistoryMessages(conversationId: string, currentUserMessage: string, currentAttachments: AttachmentMeta[]): Promise<Array<{ role: 'system' | 'user' | 'assistant'; content: string }>> {
  ...
  if (estimateTokens(messages) > MAX_TOKENS) {
    let kept = 0; let cutIndex = messages.length
    for (let index = messages.length - 1; index >= 0; index -= 1) { const tokens = Math.ceil(String(messages[index].content ?? '').length / 4); if (kept + tokens > KEEP_TOKENS) { cutIndex = index + 1; break }; kept += tokens; cutIndex = index }
...
/**
 * Canonical model execution compatibility wrapper: always calls runCanonicalLlm directly and
 * reshapes the result into an OpenAI chat-completion object. Used by the many callers that need a
 * raw inference call in that legacy shape (orchestrator.ts's tool loop, subagents.ts, and other
 * lib/ modules), as opposed to agent-canonical-bridge.ts's runOwnerAwareLlm, which additionally
 * decides whether a turn needs the full CEO cognitive lifecycle. Provider Gateway Phase B
 * (2026-09-19) renamed that other function specifically to end the "two functions named
 * callLlmWithRetry, doing different things" ambiguity this comment used to have to explain --
 * callLlmWithRetry unambiguously means this one now.
 */
export async function callLlmWithRetry(messages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }>, opts?: { thinking?: boolean }): Promise<any> {
  ...
  try {
    ...
    throw error
  } finally { RATE_LIMIT_INFO.retryingNow = false }
}
