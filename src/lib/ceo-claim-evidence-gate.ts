...
function claimScope(sentence: string): ClaimVerification['scope'] | null {
  ...
  if (INTERNAL_CLAIM_RE.test(sentence)) return 'internal_state'
  return null
}
function markerIds(sentence: string): string[] {
  return [...sentence.matchAll(/\[(S\d+-[0-9a-f]+|SEC-[A-Z0-9]+|PAGE-\d+)\]/gi)].map((match) => match[1])
}
function overlapScore(sentence: string, source: EvidenceSource): number {
  ...
  const matches = wanted.filter((token) => sourceTokens.has(token)).length
...
function matchingCandidates(sentence: string, bundle: EvidenceBundle): EvidenceClaimCandidate[] {
  ...
  return bundle.claims.filter((candidate) => {
    const ct = tokens(candidate.claim)
    const shared = wanted.filter((token) => ct.includes(token)).length
...
export function verifyClaimEvidence(content: string, bundle?: EvidenceBundle): { passed: boolean; claims: ClaimVerification[]; supportedClaimCount: number; requiredClaimCount: number } {
  ...
  for (const sentence of content.split(/[.!?\n]+/).map((s) => s.trim()).filter(Boolean)) {
    ...
    const fresh = uniqueSources.some((source) => Date.now() - source.retrievedAt >= 0 && Date.now() - source.retrievedAt <= bundle.freshness.maxAgeMs)
    const supported = uniqueSources.length > 0 && fresh && (markerSources.length > 0 || Math.max(...uniqueSources.map((source) => overlapScore(sentence, source)), 0) >= 0.28)
    claims.push({ claim: sentence.slice(0, 500), scope, supported, sourceIds: uniqueSources.map((source) => source.id), sourceTiers: uniqueSources.map((source) => source.sourceTier), reason: supported ? 'Claim maps to fresh evidence in the supplied bundle.' : 'No sufficiently fresh, matching source was found for this claim.' })
  }
  ...
  return { passed: supportedClaimCount === claims.length, claims, supportedClaimCount, requiredClaimCount: claims.length }
}