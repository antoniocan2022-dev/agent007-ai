...
function normalizeNumber(value: string, unit?: string): string {
  ...
  const multiplier = normalizedUnit === 'b' || normalizedUnit === 'bn' || normalizedUnit === 'billion' ? 1_000_000_000
    : normalizedUnit === 'm' || normalizedUnit === 'mn' || normalizedUnit === 'million' ? 1_000_000
    : normalizedUnit === 'k' || normalizedUnit === 'thousand' ? 1_000
    : 1
...
export function verifyClaimEvidence(content: string, bundle?: EvidenceBundle): { passed: boolean; claims: ClaimVerification[]; supportedClaimCount: number; requiredClaimCount: number } {
  ...
  for (const sentence of content.split(/[.!?\n]+/).map((s) => s.trim()).filter(Boolean)) {
    ...
    const fresh = uniqueSources.some((source) => {
      const age = Date.now() - source.retrievedAt
    ...
    claims.push({
      ...
      reason: supported
        ...
        : quantitativeSupport
          ? 'No sufficiently fresh, matching source was found for this claim.'
