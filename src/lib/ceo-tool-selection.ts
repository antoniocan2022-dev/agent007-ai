...
function scoreTool(tool: ToolDescriptor, need: { operation: string; requiresFreshness: boolean }): ToolScore {
  ...
  const verification = need.operation === 'verify' ? Math.min(1, tool.reliability + 0.05) : tool.reliability
  const freshness = need.requiresFreshness ? tool.freshness : 0.8
  const total = 0.23 * relevance + 0.18 * tool.reliability + 0.14 * freshness + 0.08 * latency + 0.08 * (1 - Math.min(1, tool.cost / 100)) + 0.09 * permissions + 0.10 * (1 - tool.risk) + 0.10 * verification
  return { relevance, reliability: tool.reliability, freshness, latency, cost: tool.cost, permissions, risk: tool.risk, verification, total: Number(total.toFixed(4)) }
}
...
export function selectCeoTool(contract: CeoExecutionContract, options?: { requiresFreshness?: boolean }): ToolSelection {
  ...
  for (const candidate of candidates) scores[candidate.id] = scoreTool(candidate, { operation: contract.operation, requiresFreshness: options?.requiresFreshness ?? contract.temporalScope === 'current' })
