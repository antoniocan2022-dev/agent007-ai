import { GOVERNED_MODEL_PROFILES, getGovernedCandidates, getModelForProviderGoverned, type ActiveProviderId, type GovernedModelProfile, type ModelCapability } from './provider-control-plane'
...
export function selectModelForTask(taskType: TaskType, availableProviders: readonly ProviderId[], verification?: VerificationTier): ModelSelection[] {
  ...
  for (const provider of availableProviders.filter((item): item is ActiveProviderId => item !== 'openai')) {
    ...
    for (const model of candidates) {
      ...
      if (!profile || !process.env[providerEnv(provider)]?.trim()) continue
      ...
      selections.push({
        ...
        rationale: `${taskType}: governed model ${model}; quality ${profile.quality}; speed ${profile.speed}; cost tier ${profile.costTier}`,
      })
    }
  }
...
function providerEnv(provider: ActiveProviderId): string {
  return {
    groq: 'GROQ_API_KEY',
    zai: 'ZAI_API_KEY',
...
export function getModelProfile(provider: ProviderId, model: string): GovernedModelProfile | undefined {
  return provider === 'openai' ? undefined : GOVERNED_MODEL_PROFILES.find((profile) => profile.provider === provider && profile.model === model)
}

export function getModelForProvider(provider: ProviderId, taskType: TaskType, verification?: VerificationTier): string | undefined {
  return provider === 'openai' ? undefined : getModelForProviderGoverned(provider, taskType, verification)
}
