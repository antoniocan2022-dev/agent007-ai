...
function loopIdFor(recommendationId: string | null, seed: string): string { const digest = createHash('sha256').update(`${recommendationId ?? ''}|${seed}`).digest('hex').slice(0, 24); return `continuous_loop_${digest}` }
...
export async function runGovernedEvolutionCycle(): Promise<{ cycleId: string; status: 'AWAITING_APPROVAL' | 'COMPLETED'; observed: { orgIQ: number; missionCount: number; trend: string }; proposals: string[]; simulated: string[]; awaitingApproval: string[] }> {
  ...
  for (const recommendation of actionable) {
    const metric = recommendation.toLowerCase().includes('duration') || recommendation.toLowerCase().includes('slow missions') ? 'duration' : recommendation.toLowerCase().includes('correction') || recommendation.toLowerCase().includes('template') ? 'corrections' : recommendation.toLowerCase().includes('verification') ? 'verificationScore' : recommendation.toLowerCase().includes('error') || recommendation.toLowerCase().includes('self-healing') ? 'errors' : recommendation.toLowerCase().includes('tool') ? 'tools' : 'confidence'
    const direction = ['duration', 'corrections', 'errors', 'tools'].includes(metric) ? 'decrease' as const : 'increase' as const
    const existing = (await listInitiativesByStatus('simulated', 100)).find((item) => item.recommendation === recommendation && item.targetMetric === metric)
