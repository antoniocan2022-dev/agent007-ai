...
export async function runCeoCognitiveLifecycle(request: CeoCognitiveRequest): Promise<CognitiveLifecycleResult> {
  ...
  try {
    ...
    const softPassEligible = isConversational && !isGenuineOverclaim && result.content.trim().length >= 20
    ...
    if (quality.decision !== 'PASS' && softPassEligible) console.log('[ceo-soft-pass]', JSON.stringify({ intent: decisionPlan.executionContract.intent, failureReason: quality.failureReason, contentLength: result.content.length }))
