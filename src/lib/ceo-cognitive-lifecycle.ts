...
import type { CognitiveLifecycleResult, EvidenceScope, EvidenceFreshness, EvidenceState, PreRouteDecision, CeoGenerationDiagnostics, CeoIntent } from './ceo-cognitive-contract'
...
async function attemptValidatedReasoningProvider(timeoutMs: number, taskType: TaskType = 'reasoning', verification: VerificationTier = 'standard'): Promise<ValidatedAvailability> {
  ...
  if (!governedConfigured.length) return null
...
async function tryDegraded(request: CeoCognitiveRequest, reason: string, attempts: string[], responseMsBeforeDegraded: number, decisionPlan: ReturnType<typeof buildCeoDecisionPlan>, executionPlan: ReturnType<typeof buildCeoExecutionPlan>, availabilityAttempted = false, validatedAvailability: ValidatedAvailability = null, failureReason?: CeoFailureReason, generationOverride?: Partial<CeoGenerationDiagnostics>, ventureEvidence: { ventureId: string; evidence: string } | null = null, ventureEvidenceFreshness?: EvidenceFreshness): Promise<CognitiveLifecycleResult> { throwIfCeoRequestAborted(getCeoCancellationSignal()); const started = Date.now(); const { taskType: recoveryTaskType, verification: recoveryVerification } = recoveryTaskContext(request, decisionPlan); let availability = validatedAvailability; if (!availability && !availabilityAttempted) availability = await attemptValidatedReasoningProvider(Math.max(2500, (request.timeoutMs ?? decisionPlan.latencyBudgetMs) - responseMsBeforeDegraded), recoveryTaskType, recoveryVerification); console.log('[ceo-recovery-trace]', JSON.stringify({ availabilityAttemptedByCaller: availabilityAttempted, recoveryAvailable: Boolean(availability), recoveryProvider: availability?.provider ?? null, reason: reason.slice(0, 200) })); const evidenceScope = request.evidenceScope ?? (ventureEvidence ? 'live_system' : decisionPlan.executionContract.intent === 'self_assessment' ? 'internal_state' : undefined); const evidenceFreshness = request.evidenceFreshness ?? ventureEvidenceFreshness; const recoveryLiveSystemMessages = ventureEvidence ? [{ role: 'system' as const, content: `LIVE VENTURE STATE (READ ONLY):\n${ventureEvidence.evidence}\nUse these values as system evidence. Do not invent missing values, readiness, revenue, customer success, or authorization.` }] : [];
...
export async function runCeoCognitiveLifecycle(request: CeoCognitiveRequest): Promise<CognitiveLifecycleResult> {
  ...
  const executiveStateAvailable = Boolean(worldModel && (worldModel.executive.data.strategy.dataAvailable || worldModel.executive.data.risk.dataAvailable || worldModel.executive.data.customers.dataAvailable || worldModel.executive.data.resources.dataAvailable || request.strategicHorizon || request.leadershipLedger?.length))
  const decisionSynthesis = worldModel ? synthesizeExecutiveDecision({ executive: worldModel.executive.data, partners: worldModel.partners.data, leadership: worldModel.leadership.data, systemIncidents: worldModel.system.data.incidents }) : null
  ...
  try {
    ...
    if (action === 'clarify') { primary = await runCanonicalLlm({ ...stageOptions({ maxProviderAttempts: 1, maxTokens: Math.min(request.maxTokens ?? 600, 600), executionClass: 'fast' as const }), messages: [...guardianMessages, ...decisionMessages, ...request.messages, { role: 'user', content: 'Ask the minimum necessary natural clarification needed to resolve the user’s request. Return only the clarification question.' }] }) }
    else { primary = await runCanonicalLlm({ ...stageOptions(), messages: primaryMessages }); if (executionPlan.reasoningStrategy === 'multi_pass') { try { const refinement = await runCanonicalLlm({ ...stageOptions({ maxProviderAttempts: 2 }), messages: [...primaryMessages, { role: 'assistant', content: primary.content }, buildRefinementPrompt(objective, primary.content)], excludeProviders: stageExclusions(primary.provider) }); review = refinement; final = refinement; finalStage = 'refinement' } catch (error) { if (isCeoRequestAborted(error)) throw error } } else if (executionPlan.reasoningStrategy === 'independent_review') { try { review = await runCanonicalLlm({ ...stageOptions({ maxProviderAttempts: 2 }), messages: [...worldModelMessages, ...guardianMessages, ...executiveStateMessages, ...liveSystemMessages, ...readinessMessages, ...decisionMessages, { role: 'system', content: 'You are an independent verification reviewer for Agent007. Be skeptical, precise, and concise.' }, buildReviewPrompt(objective, primary.content)], excludeProviders: stageExclusions(primary.provider) }); final = await runCanonicalLlm({ ...stageOptions({ maxProviderAttempts: 2 }), messages: [...worldModelMessages, ...guardianMessages, ...executiveStateMessages, ...liveSystemMessages, ...readinessMessages, ...decisionMessages, { role: 'system', content: 'You are the final executive synthesizer for Agent007. Use the draft and independent review to produce the strongest justified answer.' }, buildSynthesisPrompt(objective, primary.content, review.content, ventureEvidence?.evidence, readinessSynthesis ? `Level ${readinessSynthesis.level} — ${readinessSynthesis.label}. ${readinessSynthesis.verified} ${readinessSynthesis.notProven}` : undefined)], excludeProviders: stageExclusions(review.provider) }); finalStage = 'synthesis' } catch (error) { if (isCeoRequestAborted(error)) throw error; review = undefined; final = undefined; finalStage = 'primary' } } }
    if (primary) primaryQuality = evaluateCeoQuality({ objective, content: sanitizeCeoContentForQualityGate(primary.content), path: decisionPlan.path, intent: decisionPlan.executionContract.intent, reviewed: false, externalExecutionSucceeded: true, evidenceProvided, evidenceScope, evidenceFreshness, evidenceBundle: request.evidenceBundle, priorTurns: request.priorConversation, relevantOlderMessages: request.relevantOlderConversation, resolvedReferences: request.canonicalContext?.references, responseAction: request.decisionContract?.responseAction, externalAgencyAvailable })
    let output = final ?? primary
    if (!output) return tryDegraded(request, 'No usable provider output was produced.', [], Date.now() - startedAt, decisionPlan, executionPlan, false, null, 'provider_unavailable', { primaryOutputProduced: false, primaryQualityDecision: 'NOT_RUN', finalOutputProduced: false, finalStage: 'none', escalationCount: 0 }, ventureEvidence, ventureEvidenceFreshness)
    let quality = evaluateCeoQuality({ objective, content: sanitizeCeoContentForQualityGate(output.content), path: decisionPlan.path, intent: decisionPlan.executionContract.intent, reviewed: Boolean(review && executionPlan.reasoningStrategy === 'independent_review'), externalExecutionSucceeded: true, evidenceProvided, evidenceScope, evidenceFreshness, evidenceBundle: request.evidenceBundle, priorTurns: request.priorConversation, relevantOlderMessages: request.relevantOlderConversation, resolvedReferences: request.canonicalContext?.references, responseAction: request.decisionContract?.responseAction, externalAgencyAvailable })
