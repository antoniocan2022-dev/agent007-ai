  ...
  if (shouldRecoverEvidenceBeforeProvider(failureReason, decisionPlan.executionContract)) {
    ...
    try {
      ...
      if (recovered.bundle.sufficient || recovered.bundle.sources.length > currentSourceCount) {
        recoveredEvidenceBundle = recovered.bundle
        recoveredEvidenceContext = renderEvidenceBundleForPrompt(recovered.bundle)
        recoveredEvidenceScope = recovered.bundle.scope === 'mixed' ? 'mixed' : 'external_web'
        recoveredEvidenceFreshness = recovered.bundle.freshness
      }
