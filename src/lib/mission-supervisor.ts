...
export type MissionSupervisorAction =
  | 'RUN_LEADER'
  | 'ADVANCE_STAGE'
  | 'WAIT_FOR_ARTIFACT'
  | 'REPLAN_REQUIRED'
...
async function getOperatorUserId(): Promise<string | null> {
  try {
    return (await db.user.findFirst({ orderBy: { createdAt: 'asc' } }))?.id ?? null
  } catch {
    return null
  }
}
...
function activeHandoff(mission: ActiveMission) {
  return mission.chain.find((handoff) => handoff.stage === mission.currentStage) ?? null
}
...
async function advanceDurableMission(mission: ActiveMission): Promise<ActiveMission> {
  const currentIndex = ['PLANNED', 'IN_PROGRESS', 'REVIEW', 'DELIVERED', 'VERIFIED', 'OWNER_APPROVAL', 'COMPLETED'].indexOf(mission.currentStage)
  if (currentIndex < 0 || currentIndex >= 6) return mission
  ...
  const nextStage = ['PLANNED', 'IN_PROGRESS', 'REVIEW', 'DELIVERED', 'VERIFIED', 'OWNER_APPROVAL', 'COMPLETED'][currentIndex + 1] as MissionStage
  ...
  mission.log.push({
    timestamp: new Date().toISOString(),
    actor: current.leader || 'MISSION_SUPERVISOR',
    stage: mission.currentStage,
    message: `Supervisor advanced mission after verified completion: ${current.stage} → ${nextStage}.`,
  })
  await saveActiveMissionDB(mission)
  return mission
}
...
function buildLeaderTask(mission: ActiveMission, leaderId: string, leaderName: string, requiredType: string): string {
  ...
  const artifact = handoff?.artifactRequired ?? 'none'
  return [
    ...
    '',
    'Execute the next useful step that is within your authority and available capabilities.',
    'Do not claim completion without producing or referencing the required artifact.',
    'If blocked, state the exact blocker, what evidence is missing, and the smallest next action that would unblock the stage.',
    'Return a concise execution report for the CEO/Mission Supervisor, including accomplishments, blocker, artifact, verification needed, and next action.',
  ].join('\n')
}
...
export async function inspectMission(mission: ActiveMission, options?: { staleMinutes?: number }): Promise<MissionSupervisorDecision> {
  ...
  if (mission.currentStage === 'COMPLETED') {
    return { missionId: mission.id, stage: mission.currentStage, action: 'COMPLETED', reason: 'Mission is already completed.', leaderId: handoff?.team ?? null, leaderName: handoff?.leader ?? null, stale: false, capabilityReady: true, artifactRequired: handoff?.artifactRequired ?? null, artifactVerified: handoff?.artifactVerified ?? true, state }
  }

  if (!handoff) {
    return { missionId: mission.id, stage: mission.currentStage, action: 'REPLAN_REQUIRED', reason: 'Current mission stage has no registered handoff.', leaderId: null, leaderName: null, stale: true, capabilityReady: false, artifactRequired: null, artifactVerified: false, state }
  }

  const readiness = await assessMissionCapabilityReadiness(mission.currentStage, handoff.team)
  if (!readiness.ready) {
    return { missionId: mission.id, stage: mission.currentStage, action: 'REPLAN_REQUIRED', reason: `Leader capability readiness failed: ${readiness.missing.join(', ')}.`, leaderId: handoff.team, leaderName: handoff.leader, stale, capabilityReady: false, artifactRequired: handoff.artifactRequired, artifactVerified: handoff.artifactVerified, state }
  }

  if (handoff.artifactRequired !== 'none' && handoff.artifactValue && !handoff.artifactVerified) {
    return { missionId: mission.id, stage: mission.currentStage, action: 'WAIT_FOR_ARTIFACT', reason: 'Artifact exists but is not verified; do not advance until verification succeeds.', leaderId: handoff.team, leaderName: handoff.leader, stale, capabilityReady: true, artifactRequired: handoff.artifactRequired, artifactVerified: false, state }
  }

  if (handoff.artifactRequired !== 'none' && handoff.artifactVerified) {
    return { missionId: mission.id, stage: mission.currentStage, action: 'ADVANCE_STAGE', reason: 'Required stage artifact is verified.', leaderId: handoff.team, leaderName: handoff.leader, stale: false, capabilityReady: true, artifactRequired: handoff.artifactRequired, artifactVerified: true, state }
  }
  ...
  for (const mission of missions) {
    ...
    try {
      if (decision.action === 'ADVANCE_STAGE') {
        ...
        state.lastAction = updated.currentStage === 'COMPLETED' ? 'COMPLETED' : 'ADVANCE_STAGE'
        state.totalActions += 1
        state.consecutiveFailures = 0
        state.blocker = null
        state.nextAction = updated.currentStage === 'COMPLETED' ? null : 'Run the newly active stage leader.'
        advanced += 1
        acted += 1
      } else if (decision.action === 'WAIT_FOR_ARTIFACT') {
        const handoff = activeHandoff(mission)
        state.lastActionAt = new Date().toISOString()
        state.lastAction = 'WAIT_FOR_ARTIFACT'
        state.totalActions += 1
        state.blocker = handoff?.artifactVerifyError ?? 'Artifact requires verification.'
        state.nextAction = 'Verify or replace the stage artifact before advancing.'
        blocked += 1
        acted += 1
      } else if (decision.action === 'REPLAN_REQUIRED') {
        await appendSystemLog(mission, `MISSION SUPERVISOR REPLAN REQUIRED — ${decision.reason}`)
        state.lastActionAt = new Date().toISOString()
        state.lastAction = 'REPLAN_REQUIRED'
        state.totalActions += 1
        state.consecutiveFailures = Math.min(10, state.consecutiveFailures + 1)
        state.blocker = decision.reason
        state.nextAction = 'CEO/VID must select a compliant replacement leader or revise the stage requirement.'
        blocked += 1
        acted += 1
        ...
        if (leaderRuns >= (options?.maxLeaderRuns ?? DEFAULT_MAX_LEADER_RUNS_PER_TICK)) {
          ...
          if (artifact && handoff) {
            handoff.artifactValue = artifact
            handoff.artifactVerified = false
            handoff.artifactVerifiedAt = null
            handoff.artifactVerifyError = 'Artifact detected from leader output; verification is required before advancement.'
          }
          ...
          await saveActiveMissionDB(mission)
          state.lastActionAt = new Date().toISOString()
          state.lastAction = 'RUN_LEADER'
          state.totalActions += 1
          state.consecutiveFailures = 0
          state.blocker = null
          state.nextAction = handoff?.artifactRequired === 'none' ? 'Supervisor can evaluate stage completion on the next tick.' : 'Verify the produced artifact before advancing.'
          leaderRuns += 1
          acted += 1
        }
      ...
      state.nextAction = 'Retry after the blocker is resolved; escalate after repeated failures.'
      blocked += 1
    }
...
export async function getMissionSupervisorState(missionId: string): Promise<MissionSupervisorState> {
  return readState(missionId)
}
