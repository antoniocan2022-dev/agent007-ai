...
// Long-document incident (2026-09-19): every regex below now scans the bounded instruction window (see
// extractInstructionWindow), not the raw currentMessage -- a long pasted document that happens to use
...
export async function interpretCeoSemantics(context: CanonicalConversationContext, signal?: AbortSignal): Promise<Partial<SemanticInterpretation>> {
  ...
  try {
    ...
    // instead of a flat first-4,000-char slice -- the old head-only slice would silently miss the user's
    // own framing entirely when it sat at the END of a long paste (a very common pattern: paste the
    // document, then ask the question). The window covers both ends, and stays cheap for this
    // lightweight classification call regardless of how large the underlying document is.
