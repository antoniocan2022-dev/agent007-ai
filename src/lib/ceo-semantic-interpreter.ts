...
export async function interpretCeoSemantics(context: CanonicalConversationContext, signal?: AbortSignal): Promise<Partial<SemanticInterpretation>> {
  ...
  try {
    const result = await runCanonicalLlm({ messages: [
      { role: 'system', content: ['You are Agent007 semantic interpreter. Interpret the user message using the supplied conversation context.', 'Correct obvious spelling/typing noise silently. Resolve intended meaning from context without inventing facts.', 'Return ONLY valid JSON with keys: meaning, confidence, intent, speechAct, cognitiveDepth, uncertainty.', 'confidence must be a number from 0 to 1.', 'uncertainty must be an array of {code,description,severity}, severity low|medium|high.', 'Allowed intent: conversation, analysis, decision, research, action, unknown.', 'Allowed speechAct: social, question, proposition, continuation, correction, request, unknown.', 'Allowed cognitiveDepth: direct, contextual, deep, strategic.'].join(' ') },
