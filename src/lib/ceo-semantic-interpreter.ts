...
function shouldAssist(context: CanonicalConversationContext): boolean {
  const unresolvedReference = context.references.some((reference) => reference.ambiguous || reference.confidence < 0.7)
  return unresolvedReference || TYPO_HINT_RE.test(context.currentMessage) || TARGETED_REFERENCE_HINT_RE.test(context.currentMessage)
}
