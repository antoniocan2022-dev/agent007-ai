...
export type BusinessType =
  | 'saas' | 'affiliate' | 'digital_product' | 'content' | 'consulting'
  | 'etsy' | 'api' | 'subscription' | 'marketplace' | 'other'
...
function parseBusiness(value: string): Business | null {
  try { return JSON.parse(value) as Business } catch { return null }
}
...
async function findBusinessByName(name: string): Promise<Business | null> {
  ...
  return records.map((record) => parseBusiness(record.value)).find((business): business is Business =>
    business !== null && normalizeBusinessName(business.name) === normalized,
  ) ?? null
}

/** Create a business once; all callers share the same normalized-name invariant. */
export async function createBusiness(input: {
  ...
  try {
    await db.memory.create({
      data: { key: business.businessId, value: JSON.stringify(business), category: 'business_portfolio' },
    })
    return business
  } catch (error) {
...
/** Update only persisted business state and derived financial fields. */
...
/** Idempotent retirement primitive. Strategic kill policy is owned by Venture OS. */
    ...
    await db.memory.create({
      data: {
        ...
        value: JSON.stringify({
          ...
          lesson: `Business "${business.name}" (${business.type}) retired at ROI ${business.roi}%.`,
...
/** Financial safety backstop. Venture OS remains the strategic decision layer. */
...
function insightKey(sourceBusinessId: string, targetType: BusinessType, kind: string): string {
  return `cross_insight:${sourceBusinessId}:${targetType}:${kind}`
}
...
export async function shareBusinessInsights(sourceBusinessId: string): Promise<CrossBusinessInsight[]> {
  ...
  for (const type of insightTypes) {
    ...
    for (const target of targets) {
      ...
      const record: CrossBusinessInsight = {
        insightId: key,
...
export async function getCrossBusinessInsights(limit: number = 50): Promise<CrossBusinessInsight[]> {
  ...
  return records.map((record) => parseBusiness(record.value) as unknown as null)
    .map((_, index) => {
      try { return JSON.parse(records[index].value) as CrossBusinessInsight } catch { return null }
    })
...
export async function getActiveBusinesses(): Promise<Business[]> {
  return (await getPortfolio()).filter((business) => business.lifecycle !== 'retired')
}
...
export async function computeEnterpriseValue(): Promise<EnterpriseValue> {
  ...
  const totalValue = Math.round(
    recurringRevenue * 0.25 + businessAssets * 0.15 + avgAutomation * 0.15 +
    knowledgeGrowth * 0.10 + customerGrowth * 0.15 + avgBrand * 0.10 + organizationalIQ * 0.10,
  )
...
export async function runFlywheel(mode: 'full' | 'single' = 'full', startStage?: FlywheelStage): Promise<FlywheelResult> {
  ...
  const result: FlywheelResult = {
    ...
    stages: FLYWHEEL_STAGES.map(({ stage }) => ({ stage, status: 'pending' as const, output: '' })),
    opportunity: null,
    businessId: null,
    estimatedValue: null,
    validated: null,
  ...
  for (const { stage, description } of stagesToRun) {
    ...
    try {
      entry.output = await executeFlywheelStage(stage, result, description)
      entry.status = 'complete'
    } catch (error) {
      entry.status = 'complete'
      entry.output = `Error: ${error instanceof Error ? error.message.slice(0, 120) : 'unknown error'}`
      result.learnings.push(`Stage ${stage} encountered an error.`)
    }
  ...
  try {
    await db.memory.create({ data: { key: cycleId, value: JSON.stringify(result), category: 'flywheel_cycle' } })
  } catch {}
  return result
}
...
async function executeFlywheelStage(stage: FlywheelStage, context: FlywheelResult, _description: string): Promise<string> {
  switch (stage) {
    case 'observe':
      return 'Observation stage completed; external market-research capabilities should provide the evidence feed.'
    case 'identify_opportunity': {
      const completion = await callLlmWithRetry([
        { role: 'system', content: 'You are Agent007 Venture Studio. Identify one specific digital business opportunity suitable for rapid evidence-based validation. Do not claim validation or revenue.' },
        { role: 'user', content: 'Identify the strongest current opportunity candidate and explain the target customer problem.' },
      ])
    ...
    case 'estimate_value':
      return 'Value estimate requires explicit assumptions and evidence; no synthetic numeric score is generated by the Flywheel.'
    case 'validate':
      context.validated = false
      return 'Validation is pending verified market evidence. The Flywheel will not self-certify demand.'
    case 'build_mvp':
    ...
    case 'acquire_customers':
      return context.businessId ? 'Acquisition experiment definition created. Launch and customer status require verified external evidence.' : 'Customer acquisition skipped — no business exists.'
    case 'learn':
      context.learnings.push('Customer feedback, conversion, retention and satisfaction must be recorded as external evidence.')
      return 'Learning requirements recorded; no synthetic customer outcome created.'
    case 'automate':
      if (context.businessId) {
        await updateBusiness(context.businessId, { automationLevel: 40 })
        return 'Baseline automation state recorded at 40%; production automation evidence can raise this value.'
      }
      return 'Automation skipped — no business exists.'
    case 'scale':
      return context.businessId ? 'Scale gate reached. Real revenue/customer evidence is required before scaling lifecycle or metrics.' : 'Scaling skipped — no business exists.'
    case 'standardize':
      context.learnings.push('Only verified successful patterns should be standardized for reuse.')
      return 'Standardization requirements recorded.'
    case 'teach_organization':
      try {
        ...
        return 'Knowledge transfer unavailable; no false success claim emitted.'
      }
  }
}
...
export interface DualMission {
  leaderId: string
  missionA: { description: string; kpis: string[] }
  missionB: { description: string; kpis: string[] }
}
