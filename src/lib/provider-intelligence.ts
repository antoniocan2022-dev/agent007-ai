...
export type ProviderAvailabilityStatus = 'NOT_CONFIGURED' | 'CIRCUIT_OPEN' | 'UNKNOWN' | 'HEALTHY' | 'DEGRADED' | 'UNHEALTHY'
// Provider Gateway Phase A (2026-09-19): getHealthScore() returns a bare 50 for a provider with zero
// observations -- a real number, indistinguishable from an actually-mediocre provider to any consumer
// that maps score->label without separately checking totalCalls===0. getProviderMetadataSummary already
// did that check inline; this pulls it out into one reusable function so every OTHER consumer (health
// endpoints, dashboards, diagnostics) gets the same "no evidence yet is not the same as degraded"
// distinction for free instead of re-deriving (or forgetting to re-derive) it themselves.
export function getProviderAvailabilityStatus(provider: ActiveProviderId): ProviderAvailabilityStatus {
  const configured = Boolean(process.env[PROVIDER_RUNTIME_CONFIG[provider].apiKeyEnv]?.trim()) && (!PROVIDER_RUNTIME_CONFIG[provider].accountIdEnv || Boolean(process.env[PROVIDER_RUNTIME_CONFIG[provider].accountIdEnv!]?.trim()))
  if (!configured) return 'NOT_CONFIGURED'
  if (isCircuitOpen(provider)) return 'CIRCUIT_OPEN'
  const health = ensureHealth(provider)
  if (health.totalCalls === 0) return 'UNKNOWN'
  const score = getHealthScore(provider)
  return score >= 80 ? 'HEALTHY' : score >= 50 ? 'DEGRADED' : 'UNHEALTHY'
}
...
export function getProviderMetadataSummary(): string {
  ...
  for (const provider of PROVIDER_ORDER) {
    const health = ensureHealth(provider); const status = getProviderAvailabilityStatus(provider).replace('_', ' '); const model = health.currentModel || PROVIDER_RUNTIME_CONFIG[provider].defaultModel; const cacheState = catalog[provider].cached ? `catalog cached ${Math.max(0, Math.round((catalog[provider].ageMs ?? 0) / 1000))}s` : 'catalog not cached'; const successRate = health.totalCalls ? `${Math.round(health.successCount / health.totalCalls * 100)}% success` : 'no runtime data'; const latency = health.avgResponseMs ? `${health.avgResponseMs}ms avg` : 'no latency data'; lines.push(`- ${PROVIDER_RUNTIME_CONFIG[provider].label}: ${status} | model: ${model} | ${successRate} | ${latency} | ${cacheState}`)
  }
