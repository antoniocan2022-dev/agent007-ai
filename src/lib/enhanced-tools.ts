...
type LlmResult = { ok: true; content: string } | { ok: false; error: string }

// Fresh-audit fix (round 2): this used to swallow every failure -- primary AND fallback -- into the
// bare string '(LLM unavailable)', with no detail about what actually went wrong. Every call site
// below then appended a "CAPABILITY STATUS: ... active" (or, in one case, "full access, no
// limitations") line unconditionally, regardless of whether that string was real analysis or the
// failure placeholder. A discriminated result lets every call site report honestly instead.
async function llm(systemPrompt: string, userPrompt: string, maxTokens = 1500): Promise<LlmResult> {
  ...
  try {
    ...
    primaryError = 'primary LLM returned no content'
  } catch (e: any) { primaryError = e?.message ?? String(e) }
  try {
    ...
    if (content.trim()) return { ok: true, content }
    return { ok: false, error: `primary failed (${primaryError}); fallback returned no content` }
  } catch (e: any) { return { ok: false, error: `primary failed (${primaryError}); fallback failed (${e?.message ?? String(e)})` } }
}

// Fresh-audit fix (round 2): renders the honest CAPABILITY STATUS line every call site below used to
// hardcode as unconditionally "active" (or, in advanced_data_analysis, an unqualified blanket
// capability claim), regardless of whether the LLM call above it actually succeeded.
...
export async function toolAdvancedDataAnalysis(args: { data_type?: string; timeframe?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok('Advanced data analysis complete', reportFrom('Enhanced analytics', llmResult))
...
export async function toolPredictiveAnalyticsIncome(args: { forecast_days?: number }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`Income forecast: ${days} days`, reportFrom('Predictive analytics', llmResult))
...
export async function toolMarketTrendInsights(args: { industry?: string; region?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    try {
      const canonicalLlm = await getCanonicalLlmBridge()
      const results = await canonicalLlm.functions.invoke('web_search', { query: `${industry} market trends 2025 ${region}`, num: 5 })
    ...
    return ok('Market trend insights generated', reportFrom('Market intelligence', llmResult))
...
export async function toolUserBehaviorAnalysis(args: { segment?: string }, _ctx: ToolContext): Promise<ToolResult> {
  try {
    ...
    return ok('User behavior analysis complete', reportFrom('Behavior analytics', llmResult))
...
export async function toolEmailMarketingAutomation(args: { campaign_type?: string; audience?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok('Email marketing sequence designed', reportFrom('Email marketing automation', llmResult))
...
export async function toolSocialMediaManagement(args: { platforms?: string; goal?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok('Social media strategy designed', reportFrom('Social media management', llmResult))
...
export async function toolSocialMediaScheduler(args: { content?: string; platforms?: string; frequency?: string }, _ctx: ToolContext): Promise<ToolResult> {
  try {
    ...
    return ok('Social media schedule created', reportFrom('Social scheduler', llmResult))
...
export async function toolConversionOptimizer(args: { current_rate?: number; target_rate?: number }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`Conversion plan: ${currentRate}% → ${targetRate}%`, reportFrom('Conversion optimization', llmResult))
...
export async function toolPortfolioOptimizer(args: { risk_tolerance?: string; capital?: number }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`Portfolio optimized ($${capital}, ${risk} risk)`, reportFrom('Portfolio optimization', llmResult, '\n\n⚠ This is educational analysis, not investment advice. Consult a licensed financial advisor.'))
...
export async function toolRealtimeMarketData(args: { assets?: string; market?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    try {
      const canonicalLlm = await getCanonicalLlmBridge()
      const results = await canonicalLlm.functions.invoke('web_search', { query: `${assets} price today ${market} market`, num: 5 })
    ...
    return ok('Real-time market data retrieved', reportFrom('Market data feed', llmResult))
...
export async function toolCryptoAnalyzer(args: { coin?: string; analysis_type?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    try {
      const canonicalLlm = await getCanonicalLlmBridge()
      const results = await canonicalLlm.functions.invoke('web_search', { query: `${coin} crypto analysis price prediction 2025`, num: 5 })
      searchData = JSON.stringify(results?.results ?? '').slice(0, 2000)
    } catch {}
    const llmResult = await llm(
    ...
    return ok(`${coin} analysis complete`, reportFrom('Crypto analyzer', llmResult))
...
export async function toolStockScreener(args: { sector?: string; criteria?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    try {
      const canonicalLlm = await getCanonicalLlmBridge()
      const results = await canonicalLlm.functions.invoke('web_search', { query: `best ${sector} stocks 2025 ${criteria} screener`, num: 5 })
      searchData = JSON.stringify(results?.results ?? '').slice(0, 2000)
    } catch {}
    const llmResult = await llm(
    ...
    return ok(`Stock screen: ${sector} (${criteria})`, reportFrom('Stock screener', llmResult))
...
export async function toolAIWritingAssistant(args: { topic?: string; format?: string; tone?: string; word_count?: number }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`${format} written: ${topic}`, reportFrom('AI writing assistant', llmResult))
...
export async function toolSEOOptimizer(args: { content?: string; target_keyword?: string; url?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`SEO analysis: "${keyword}"`, reportFrom('SEO optimizer', llmResult))
...
export async function toolContentCalendarGenerator(args: { niche?: string; duration_weeks?: number }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`Content calendar: ${weeks} weeks`, reportFrom('Content calendar generator', llmResult))
...
export async function toolContentRepurposer(args: { source_content?: string; source_format?: string; target_formats?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`Content repurposed: ${sourceFormat} → ${targetFormats}`, reportFrom('Content repurposer', llmResult))
...
export async function toolNicheDiscoveryAgent(args: { market?: string; budget?: number; skills?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    try {
      const canonicalLlm = await getCanonicalLlmBridge()
      const results = await canonicalLlm.functions.invoke('web_search', { query: `profitable niches 2025 ${market} low competition high demand`, num: 5 })
      searchData = JSON.stringify(results?.results ?? '').slice(0, 2000)
    } catch {}
    const llmResult = await llm(
    ...
    return ok('Niche discovery complete', reportFrom('Niche discovery agent', llmResult))
...
export async function toolBudgetForecaster(args: { timeframe_months?: number; income?: number; expenses?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    return ok(`Budget forecast: ${months} months`, reportFrom('Budget forecasting', llmResult))
...
export async function toolTaxOptimizer(args: { country?: string; income?: number; entity_type?: string }, _ctx: ToolContext): Promise<ToolResult> {
  ...
  try {
    ...
    try {
      const canonicalLlm = await getCanonicalLlmBridge()
      const results = await canonicalLlm.functions.invoke('web_search', { query: `tax optimization ${country} ${entityType} ${income} deductions 2025`, num: 5 })
      searchData = JSON.stringify(results?.results ?? '').slice(0, 2000)
    } catch {}
    const llmResult = await llm(
    ...
    return ok(`Tax optimization: ${country} ($${income})`, reportFrom('Tax optimizer', llmResult, '\n\n⚠ This is informational, not tax advice. Consult a licensed CPA.'))
