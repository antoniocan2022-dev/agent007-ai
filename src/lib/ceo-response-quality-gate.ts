...
export function scoreCeoConversationQuality(input: {
  ...
  const continuity = prior.length
    ? scoreContextContinuity({ currentUserMessage: objective, response: content, priorTurns: prior, relevantOlderMessages: input.relevantOlderMessages }).score
    : 70
  const wanted = new Set(normalize(objective))
  const answer = new Set(normalize(content))
...
export function evaluateCeoQuality(input: {
  ...
  // Never infer "conversation" merely because intent is omitted. Missing intent must remain governed.
  ...
  const continuity: ContextContinuityScore | undefined = !conversational && input.priorTurns?.length
  ...
  const continuityScore = conversationQuality?.continuity ?? continuity?.score ?? 100
  const continuityOk = conversational ? continuityScore >= 55 : (continuity ? continuity.understood : true)
  ...
  return {
    decision: passed ? 'PASS' : conversational ? 'ESCALATE' : input.path === 'fast' ? 'DEGRADED' : 'ESCALATE',
    evidenceState,
    verificationStatus,
