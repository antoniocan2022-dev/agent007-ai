...
export function isObjectiveAgreementContinuationRequest(text: string): boolean {
  ...
  // An explicit continuation cue is only sufficient when it is terminal or explicitly refers to the
  // existing thread/objective. This prevents "Yes, go ahead. Tell me about the weather" from inheriting
  // an unrelated active objective.
  if (new RegExp(`(?:${EXPLICIT_CONTINUATION_CUE_RE.source})(?:\\s+(?:with|on|from)\\s+(?:this|that|it|these|those|the same|the thread|the topic|the plan|the request))?\\s*[.!?]*const RETROSPECTIVE_REQUEST_RE = /\b(?:why did (?:we|i)|what did we|what were we|what was the|how did we arrive at|where did we land on|remind me why|why was)\b[^?]{0,180}\b(?:choose|chose|decide|decided|select|selected|pick|picked|reason|reasoning|rationale|decision|plan|priority|name|called)\b/i
const EXPLICIT_RETROSPECTIVE_RE = /\b(?:what did we decide|what did we discuss|what did we choose|why did we choose|why did we decide|what was the reasoning|what led us to|how did we arrive at)\b/i
const DIRECT_CORRECTION_RE = /^\s*(?:that's\s+(?:not|n't)\b|that is\s+not\b|i\s+mean\b|what\s+i\s+meant\b|correction\b)/i
const NEGATED_CORRECTION_RE = /^\s*no\s*(?:,|-|:)\s*(?=(?:i|we|the|that|this|it|my|our|instead|rather)\b)/i
const CURRENT_TOPIC_REQUEST_RE = /\b(?:what are we discussing(?: now)?|what(?:'s| is)\s+(?:the\s+)?(?:current\s+)?(?:topic|subject)|what is this about)\b/i
