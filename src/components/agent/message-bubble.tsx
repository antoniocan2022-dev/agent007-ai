...
function MessageBubbleComponent({ message }: { message: ChatMessage }) {
  ...
  const anchorId = `chat-message-${message.id}`

  if (isUser) {
    return (
      <motion.div id={anchorId} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="flex justify-end gap-3 mb-6 scroll-mt-24">
        <div className="flex-1 flex flex-col items-end max-w-[85%]">
          ...
          <div className="glass rounded-2xl rounded-tr-sm px-4 py-3 text-sm text-[#e0e7ff]">
            ...
            {message.attachments && message.attachments.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{message.attachments.map((a, i) => <AttachmentChip key={i} att={a} />)}</div>}
          </div>
        </div>
        <div className="flex-shrink-0 w-9 h-9 rounded-full bg-cyan-400/10 border border-cyan-400/30 flex items-center justify-center text-cyan-300"><User className="w-4 h-4" /></div>
      </motion.div>
    )
  }
  ...
  return (
    <motion.div id={anchorId} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="flex gap-3 mb-8 scroll-mt-24">
      <div className="flex-shrink-0 mt-1"><HexAvatar size={36} pulse={isStreaming} /></div>
      <div className="flex-1 min-w-0 max-w-[760px]">
        <div className="text-[10px] label-tag mb-1 flex items-center gap-2">Agent007 AI{isStreaming && <span className="inline-flex items-center gap-1 text-[9px] text-cyan-300"><span className="typing-dot" /><span className="typing-dot" /><span className="typing-dot" /></span>}</div>
