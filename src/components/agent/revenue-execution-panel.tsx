  ...
  const approve = async (id: string) => {
    ...
    try {
      const response = await fetch('/api/revenue-execution', {
        ...
        body: JSON.stringify({ operation: 'approve', actionId: id }),
      })
      const data = await response.json().catch(() => null)
    ...
    <div className="rounded-2xl glass border border-cyan-400/15 p-4 sm:p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[11px] text-[#7181aa]">Durable execution intent and approvals. Approval is not revenue.</p>
        </div>
          ...
          {actions.map((action) => (
            <div key={action.id} className={`rounded-xl border p-3 ${statusClass(action.status)}`}>
              ...
              {action.status === 'pending' && (
                <button onClick={() => void approve(action.id)} disabled={workingId === action.id} className="mt-3 w-full h-8 rounded-lg border border-emerald-400/20 bg-emerald-400/[0.05] text-[10px] text-emerald-200 flex items-center justify-center gap-1.5 disabled:opacity-50">
              ...
              {action.status === 'approved' && <div className="mt-3 flex items-center gap-1.5 text-[9px] text-emerald-200"><LockKeyhole className="w-3 h-3" /> Approved. Awaiting a registered external executor.</div>}
            </div>
          ))}
        </div>
      )}
