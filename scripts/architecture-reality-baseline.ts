...
const required = [
  ...
  'src/lib/ceo-self-reflection.ts', 'src/lib/ceo-operator-intelligence.ts', 'src/lib/evolution-engine.ts', 'src/lib/closed-loop-improvement.ts',
...
const result = { schemaVersion: 3, generatedAt: new Date().toISOString(), fileCount: files.length, sourceFileCount: sourcePaths.length, canonicalCapabilityCount: Object.keys(CANONICAL_CAPABILITY_LEDGER).length, canonicalCapabilityLedger: CANONICAL_CAPABILITY_LEDGER, requiredArchitectureFiles: required, findings: failures, warnings, status: failures.length ? 'FAILED' : 'PASSED' }
