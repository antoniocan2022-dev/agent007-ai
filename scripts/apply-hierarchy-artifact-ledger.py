...
def replace_once(path: str, old: str, new: str, label: str) -> None:
    ...
    if count == 1:
        ...
        if new not in text:
            raise RuntimeError(f'{label}: expected anchor missing: {old[:100]!r}')
    else:
        raise RuntimeError(f'{label}: ambiguous anchor count={count}')
...
def patch_governance() -> None:
    ...
    text = p.read_text()
    if "from './hierarchy-control'" not in text:
        text = text.replace("import { PROVIDER_PRIORITY } from './provider-intelligence-policy'", "import { PROVIDER_PRIORITY } from './provider-intelligence-policy'\nimport { HIERARCHY_PARENT } from './hierarchy-control'")
    mapping = {
        'vid': 'ceo',
...
def patch_subagents() -> None:
    ...
    text = p.read_text()
    if "from './hierarchy-control'" not in text:
        anchor = "import { SHARED_MAX_PERFORMANCE_PROTOCOL } from './subagent-max-performance'"
    ...
    if return_marker in text and 'const artifact = await registerArtifact' not in text:
...
def patch_orchestrator() -> None:
    ...
    text = p.read_text()
    if "from './artifact-ledger'" not in text:
        anchor = "import { getOperatorUserId, getIncomeSettings, setIncomeSettings } from '@/lib/settings'"
        text = text.replace(anchor, anchor + "\nimport { handoffArtifact } from './artifact-ledger'")
    # Every orchestrator dispatch originates at CEO level.
    ...
    # Add explicit CEO handoff records.
    single_anchor = '        subAnswer = result.answer\n        // UPGRADE #169 C3:'
    if 'handoffArtifact(result.artifactId, \'ceo\')' not in text and single_anchor in text:
...
def patch_mission_pipeline() -> None:
    ...
    text = p.read_text()
    if "from './artifact-ledger'" not in text:
        anchor = "import { saveHeartbeat, buildHeartbeatFromAuditLog, type MissionHeartbeat } from './mission-heartbeat'"
    ...
    # Pass hierarchy + mission context into leader execution.
    ...
    # The helper needs previousArtifactId in its opts; add it if absent.
    ...
    if old in text:
        ...
        raise RuntimeError('mission-pipeline.ts: leader run anchor missing')
    # CEO stage: register the executive report as an artifact before return.
    ceo_return = '''      return {\n        output: teamOutput,\n        rounds: round,\n        finalScore: 100,'''
    ...
    if ceo_return in text and 'let ceoArtifactId' not in text:
        text = text.replace(ceo_return, ceo_repl, 1)
        text = text.replace('        artifactVerified: true,\n      }\n    }', '        artifactVerified: true,\n        artifactId: ceoArtifactId,\n      }\n    }', 1)
    # Register/verify subagent artifact once verification is known.
    ...
    if old_extract in text and 'let artifactId: string | undefined' not in text:
    ...
    # carry previous artifact ID across stages and hand it off to next leader.
...
def main() -> None:
    ...
    # Hard invariant checks after transformation.
    required = [
        'src/lib/hierarchy-control.ts',
        'src/lib/artifact-ledger.ts',
