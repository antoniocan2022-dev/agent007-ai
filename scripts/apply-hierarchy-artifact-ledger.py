...
def ensure_import(text: str, anchor: str, import_text: str, label: str) -> str:
...
def patch_governance() -> None:
    ...
    # Root executive is intentionally a literal root identifier, not a key in HIERARCHY_PARENT.
...
def patch_subagents() -> None:
    ...
    text = ensure_import(
        text,
        "import { SHARED_MAX_PERFORMANCE_PROTOCOL } from './subagent-max-performance'",
    ...
    if text == original:
        raise RuntimeError('subagents.ts: transformation made no changes')
    p.write_text(text)
