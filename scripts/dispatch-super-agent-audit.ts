/**
 * dispatch-super-agent-audit.ts
 *
 * Phase 5 of the audit chain — dispatches the Super Agent (Agent007) on Vercel
 * to redo the exhaustive audit and fix any remaining issues autonomously.
 *
 * USAGE (after deploy):
 *   1. Get a session cookie by logging in:
 *      curl -c cookies.txt -X POST https://agent007-ai.vercel.app/api/auth/callback/credentials \
 *        -H "Content-Type: application/x-www-form-urlencoded" \
 *        -d "email=antonio.can2022@hotmail.com&password=YOUR_PASSWORD&csrfToken=..."
 *
 *   2. Run this script with the session cookie:
 *      bun run scripts/dispatch-super-agent-audit.ts
 *
 *   OR (manual):
 *   - Open the dashboard at https://agent007-ai.vercel.app
 *   - Log in
 *   - Paste the AUDIT_PROMPT below into the chat
 *   - Send — Agent007 will execute the full audit + fix chain
 */
...
const PROMPT_FILE = path.join(OUTPUT_DIR, 'super-agent-audit-prompt.md')

export const AUDIT_PROMPT = `# EXHAUSTIVE AUDIT + FIX MISSION (Upgrade #58 follow-up)
...
async function main() {
  ...
  fs.writeFileSync(PROMPT_FILE, AUDIT_PROMPT)
  console.log(`Super agent audit prompt written to: ${PROMPT_FILE}`)
  console.log(`Size: ${fs.statSync(PROMPT_FILE).size} bytes`)
  console.log('')
  console.log('=== USAGE ===')
  console.log('After deploy, the owner should:')
  console.log('1. Open https://agent007-ai.vercel.app')
  console.log('2. Log in with antonio.can2022@hotmail.com')
  console.log('3. Paste the contents of super-agent-audit-prompt.md into the chat')
  console.log('4. Send — Agent007 will execute the full audit + fix chain autonomously')
  console.log('')
  console.log('Alternatively, run via API:')
  console.log('  curl -b cookies.txt -X POST https://agent007-ai.vercel.app/api/agent \\')
  console.log('    -H "Content-Type: application/json" \\')
  console.log(`    -d '{"message": $(cat ${PROMPT_FILE} | jq -Rs .), "language":"en"}'`)
}
