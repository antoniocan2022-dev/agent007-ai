...
describe('Phases A-E structural integration', () => {
  ...
  test('the canonical heartbeat and autonomy authority boundaries are singular and coordinated', () => {
    ...
    expect(workflow).toContain('bun test tests/autonomy-graduation.integration.test.ts tests/phase-d-e-integration.test.ts')
    expect(workflow).toContain('bun test tests/phase-a-e-structural-integrity.test.ts')
