...
const email = `autonomy-proof-${suffix}@example.com`
...
async function createRealizedTransaction(amount = 17.5) {
  ...
  const customer = await db.customer.create({ data: { userId: user.id, name: 'Proof Customer', email: email, status: 'customer' } })
  const transaction = await db.transaction.create({ data: { userId: user.id, provider: 'stripe', providerTxId: `pi_ci_${suffix}_${Math.random().toString(36).slice(2, 8)}`, amount, currency: 'USD', status: 'succeeded', customerEmail: email, customerName: 'Proof Customer', rawPayload: JSON.stringify({ test: true }), ventureId, customerId: customer.id } })
...
describe('Commercial autonomy proof chain', () => {
  test('realized Transaction → verified BusinessOutcome → KPI → CEO evidence', async () => {
    ...
    try {
      const outcome = await recordVerifiedTransactionOutcome({ ventureId, transactionId: transaction.id, amount: 17.5, currency: 'USD', revenueCorrelationId: `proof_${suffix}` })
  ...
  test('experiment decision activates a controlled running experiment and attribution is transaction-backed', async () => {
    ...
    let experimentId = ''
    try {
      const experiment = await activatePortfolioExperiment({ business: 'revenue-recovery', decisionId: `decision_${suffix}`, hypothesis: 'Increase verified conversion by testing a controlled offer variant.', metric: 'conversion_rate', baseline: 8, target: 10, budget: 0 })
      ...
      await db.memory.delete({ where: { key: `architecture_business_outcome:TRANSACTION:${transaction.id}` } }).catch(() => undefined)
      await db.memory.delete({ where: { key: `experiment_attribution_${experimentId}_${transaction.id}` } }).catch(() => undefined)
      await db.transaction.delete({ where: { id: transaction.id } })
      await db.customer.delete({ where: { id: customer.id } })
      await db.user.delete({ where: { id: user.id } })
    }
