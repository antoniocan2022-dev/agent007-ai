...
describe('Phase 9 world model wired into the live lifecycle', () => {
  ...
  test('route.ts fetches real partner intelligence and both CEO lanes actually receive it', () => {
    ...
    const lifecycleBlocks = source.split('runCeoCognitiveLifecycle({').slice(1)
    const worldModelLanes = lifecycleBlocks.filter((block) => /partnerIntelligence\s*,/.test(block) && /executiveState\s*,/.test(block) && /leadershipLedger\s*,/.test(block) && /strategicHorizon/.test(block))
    expect(worldModelLanes.length).toBeGreaterThanOrEqual(2)
  })
  ...
  test('route.ts fetches real executive state under the CEO Grounding Policy (self-assessment, decision/analysis intent, a recommend/decide response, mission-relevant turns, or self-inspection), not self-assessment alone, since it is backed by the heavier operational-KPI computation', () => {
    ...
    const lifecycleBlocks = source.split('runCeoCognitiveLifecycle({').slice(1)
    const groundedWorldModelLanes = lifecycleBlocks.filter((block) => /partnerIntelligence\s*,/.test(block) && /executiveState\s*,/.test(block) && /leadershipLedger\s*,/.test(block) && /strategicHorizon/.test(block))
    expect(groundedWorldModelLanes.length).toBeGreaterThanOrEqual(2)
  })
